...
def make_plot() -> plt.Figure:
    ...
    apply_style()
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(10.0, 4.2), gridspec_kw={"width_ratios": [1.15, 1.0]})
    ax1.set_xlim(0, 10)
    ax1.set_ylim(0, 6)
    ax1.axis("off")
    ax1.set_title("(a) The induction equation", fontsize=11)
    ax1.text(5.0, 5.2, r"$\dfrac{\partial \vec{B}}{\partial t} = \nabla \times (\vec{v} \times \vec{B}) + \eta \nabla^2 \vec{B}$", ha="center", va="center", fontsize=14)
    for x, fc, ec, title, body in ((2.5, "#fdebd0", "#c46b1a", "advection", "flow stretches and\nfolds field lines:\namplification, $\\sim U B / L$"),
        ...
        ax1.add_patch(FancyBboxPatch((x - 2.3, 1.4), 4.6, 2.6, boxstyle="round,pad=0.05,rounding_size=0.2", facecolor=fc, edgecolor=ec, lw=1.2))
        ax1.text(x, 3.6, title, ha="center", va="center", fontsize=11, weight="bold", color=ec)
        ax1.text(x, 2.4, body, ha="center", va="center", fontsize=10)
    ax1.add_patch(FancyArrowPatch((3.6, 4.6), (2.6, 4.05), arrowstyle="-|>", mutation_scale=10, color="0.3", lw=1.0))
    ax1.add_patch(FancyArrowPatch((7.0, 4.6), (7.4, 4.05), arrowstyle="-|>", mutation_scale=10, color="0.3", lw=1.0))
    ax1.text(5.0, 0.7, r"$\mathrm{Rm} = \dfrac{\text{advection}}{\text{diffusion}} = \dfrac{U L}{\eta}$;  a dynamo needs $\mathrm{Rm} \gtrsim \mathrm{Rm}_c \sim 10$ to $100$", ha="center", va="center", fontsize=11)
    u = np.logspace(-6, -2, 200)
    ax2.loglog(u, u * L_CORE / ETA, color="#1f6db8", lw=2.2, label=r"$\mathrm{Rm} = UL/\eta$, $L = 3.5 \times 10^6$ m, $\eta = 1$ m$^2$ s$^{-1}$")
    ax2.axhspan(10, 100, color="#f4a261", alpha=0.35, label=r"critical range $\mathrm{Rm}_c \sim 10$ to $100$")
    ax2.plot(U_EARTH, U_EARTH * L_CORE / ETA, "o", color="#c0392b", ms=8, zorder=5)
    ax2.annotate("Earth's outer core:\n" + r"$U \approx 5 \times 10^{-4}$ m s$^{-1}$, $\mathrm{Rm} \approx 1750$", xy=(U_EARTH, 1750), xytext=(1.5e-6, 1.2e4), fontsize=10, arrowprops=dict(arrowstyle="->", color="0.3", lw=0.9))
