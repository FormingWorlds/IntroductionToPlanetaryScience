...
x_earth = e_h / (2700.0 * G_EARTH)
check("(c) E/(rho_t g) Earth", x_earth, 1.6601e16)
d_earth = x_earth**0.25
check("(c) crater D Earth [m]", d_earth, 1.135e4)
check("(c) crater D Earth [km]", d_earth / 1e3, 11.35)
dt_earth = 15.0 * G_MOON / G_EARTH
check("(c) transition D Earth [km]", dt_earth, 2.477)
t_plain = 1.8e-3 / 8.38e-4
check("(d) surface age [Gyr]", t_plain, 2.148)
...
v_stokes = 2 * 4000.0 * 5.0 * 0.01**2 / (9 * 100.0)
check("(b) Stokes velocity [m/s]", v_stokes, 4.444e-3)
t_sink = 8e5 / 4.444e-3
check("(b) sinking time [s]", t_sink, 1.800e8)
check("(b) sinking time [yr] ~5.7", t_sink / YR, 5.7, rtol=1e-2)
rm = 5e-4 * 1.8e6 / 1.0
check("(c) magnetic Reynolds number", rm, 900.0)
...
expo_w = -5400.0 * (1 / 303 - 1 / 273)
check("(b) exponent", expo_w, 1.9584)
check("(b) exp factor", math.exp(1.9584), 7.088)
psat303 = 611.0 * 7.088
check("(b) P_sat(303 K) [Pa]", psat303, 4331.0)
check("(c) 1/273 [1/K]", 1 / 273, 3.6630e-3)
lnratio = math.log(1700.0 / 611.0)
check("(c) ln(e/611)", lnratio, 1.0233)
inv_td = 3.6630e-3 - 1.0233 / 5400.0
check("(c) 1/T_d [1/K]", inv_td, 3.4735e-3)
td = 1 / 3.4735e-3
check("(c) dew point [K]", td, 287.9)
z_cb = (303.0 - 287.9) / 9.8
check("(c) cloud base [km]", z_cb, 1.541)
expo_m = -980.0 * (1 / 94 - 1 / 90.7)
check("(d) exponent", expo_m, 0.3793)
psat94 = 1.17e4 * math.exp(0.3793)
check("(d) P_sat CH4(94 K) [Pa]", psat94, 1.710e4)
p_ch4 = 0.05 * 1.5e5
check("(d) CH4 partial pressure [Pa]", p_ch4, 7.5e3)
check("(d) relative humidity", p_ch4 / 1.710e4, 0.44, rtol=5e-3)

# ── Problem 5: Mercury, the shrinking planet ────────────────
print("Problem 5  Mercury, the shrinking planet")
s_merc = S0 / 0.387**2
check("(a) flux at Mercury [W/m^2]", s_merc, 9087.0)
check("(a) flux ratio to Earth ~6.7", 1 / 0.387**2, 6.7, rtol=5e-3)
arg_eq = 9087.0 * 0.912 / (4 * SIGMA)
check("(a) T_eq^4 argument", arg_eq, 3.6539e10)
check("(a) T_eq [K]", teq(9087.0, 0.088), 437.2)
arg_ss = 9087.0 * 0.912 / SIGMA
check("(a) T_ss^4 argument", arg_ss, 1.4616e11)
check("(a) T_subsolar [K]", arg_ss**0.25, 618.3)
check("(a) subsolar/global factor", 4**0.25, 1.414)
m_merc = 4 / 3 * math.pi * R_MERC**3 * RHO_MERC
check("(b) Mercury mass [kg]", m_merc, 3.302e23)
e_cool = 3.302e23 * 800.0 * 200.0
check("(b) cooling energy [J]", e_cool, 5.283e28)
t_45 = 4.5e9 * YR
check("(c) 4.5 Gyr [s]", t_45, 1.4202e17)
p_avg = 5.283e28 / 1.4202e17
check("(c) mean power [W]", p_avg, 3.720e11)
area = 4 * math.pi * R_MERC**2
check("(c) surface area [m^2]", area, 7.4815e13)
q_flux = 3.720e11 / 7.4815e13
check("(c) heat flux [W/m^2]", q_flux, 4.972e-3)
dr = 3e-5 / 3 * R_MERC * 200.0
check("(d) radius contraction [m]", dr, 4880.0)
...
t_ms = 10 * 0.7**-2.5
check("(b) main-sequence lifetime [Gyr]", t_ms, 24.39)
d_hz = math.sqrt(0.2870)
check("(c) equal-flux distance [AU]", d_hz, 0.5357)
arg_hz = 1361.0 * 0.70 / (4 * SIGMA)
check("(c) T_eq^4 argument", arg_hz, 4.2006e9)
check("(c) T_eq [K]", teq(1361.0, 0.30), 254.6)
check("(d) d^3", 0.5357**3, 0.1537)
check("(d) d^3 / (M/Msun)", 0.1537 / 0.7, 0.2196)
p_orb = math.sqrt(0.2196)
check("(d) orbital period [yr]", p_orb, 0.4686)
check("(d) orbital period [d]", 0.4686 * 365.25, 171.0, rtol=2e-3)

# ── Marks bookkeeping and verdict ───────────────────────────
check_points("mockexam03/mockexam03_content.tex")
verdict()
