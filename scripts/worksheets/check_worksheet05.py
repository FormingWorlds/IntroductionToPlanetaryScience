...
# ── (removed) deuterium and Venus's lost water ──────────────────────────
f_071 = 150.0 ** (1 / (0.71 - 1))  # printed-chain, bare-Jeans alpha

# ── Problem 3: Jeans escape from Mars ────────────────────────────────────
chk("P3a 2GM (m^3 s^-2)", 2 * 6.674e-11 * 6.4e23, 8.5427e13, 1e-4)
chk("P3a 2GM/r (m^2 s^-2)", 8.5427e13 / 3.6e6, 2.3730e7, 1e-4)
chk("P3a escape speed (m s^-1)", math.sqrt(2.3730e7), 4871.3, 1e-4)
chk("P3a escape speed (~4.9 km s^-1)", 4871.3 / 1000.0, 4.9, 1e-2)

chk("P3b GM (m^3 s^-2)", 6.674e-11 * 6.4e23, 4.2714e13, 1e-4)
chk("P3b kB T r (J m)", 1.381e-23 * 270.0 * 3.6e6, 1.3423e-14, 1e-4)
chk("P3b GM/(kB T r) (kg^-1)", 4.2714e13 / 1.3423e-14, 3.1820e27, 1e-4)
chk("P3b lambda_H", 3.1820e27 * 1.661e-27, 5.2854, 1e-4)
chk("P3b lambda_H2", 2 * 5.2854, 10.571, 1e-4)
chk("P3b lambda_CO2", 44 * 5.2854, 232.56, 1e-4)

chk("P3c exp(-lambda_H)", math.exp(-5.2854), 5.0651e-3, 1e-3)
chk("P3c exp(-lambda_H2)", math.exp(-10.571), 2.5656e-5, 1e-3)
ratio = math.sqrt(2.0) * (1 + 5.2854) * 5.0651e-3 / ((1 + 10.571) * 2.5656e-5)
chk("P3c flux ratio H/H2", ratio, 151.67, 1e-3)  # printed-chain from printed exps
chk("P3c log10 exp(-lambda_CO2) (~-101)", -232.56 / math.log(10), -101, 5e-3)

# ── Problem 4: Mercury and Mars, two ways to end ─────────────────────────
chk("P4a 1/P_rot (d^-1)", 1 / 58.65, 1.70503e-2, 1e-4)
chk("P4a 1/P_orb (d^-1)", 1 / 87.97, 1.13675e-2, 1e-4)
chk("P4a solar-day rate (d^-1)", 1.70503e-2 - 1.13675e-2, 5.6828e-3, 1e-4)
chk("P4a solar day (d)", 1 / 5.6828e-3, 175.97, 1e-4)
chk("P4a two orbits (d)", 2 * 87.97, 175.94)
chk("P4a solar day ~ two Mercury years", float(abs(175.97 - 175.94) < 0.05), 1.0, 0)

chk("P4b delta A / A", 2 * 7.0 / 2440.0, 5.7377e-3, 1e-4)
chk("P4b area change (~0.57%)", 2 * 7.0 / 2440.0 * 100, 0.57, 1e-2)
