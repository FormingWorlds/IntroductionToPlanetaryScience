...
\begin{frame}{Course Overview}
  \begin{columns}[T]
    ...
    \textbf{14 lectures + 7 tutorials} spanning:
    \begin{itemize}
...
\begin{frame}{A Pale Blue Dot}
  \begin{columns}[c]
    ...
    On 14 February 1990, Voyager~1 turned its camera back toward the inner solar system from $\sim$6 billion~km ($\sim$40~AU).
    \vskip10pt
...
\begin{frame}{Question 3: Are We Alone?}
  \centering
  \includegraphics[width=0.75\textwidth]{trappist1_lineup}
...
\begin{frame}{The Pluto Problem}
  \begin{columns}[c]
    ...
    \centering
    \includegraphics[width=0.95\textwidth]{eris_dysnomia}
    \source{ESO/L.~Cal\c{c}ada/N.~Risinger}
...
\begin{frame}{Alternative: Geophysical Definition}
  ...
  \end{itemize}
  \vskip6pt
  \keyresult{For this course, classification matters less than physics. The solar system contains a continuous spectrum of objects, from dust grains to gas giants, governed by the same physical laws.}
\end{frame}
...
\begin{frame}{Ancient \& Pre-Telescopic Astronomy}
  \begin{columns}[c]
    \column{0.35\textwidth}
    \centering
    \includegraphics[height=0.60\textheight]{ptolemaic_system}
    \source{A.~Cellarius (1660)}
    ...
    \begin{itemize}\itemsep2pt
      ...
      \item \textbf{1609--19}: Kepler's three laws, circles $\rightarrow$ ellipses
      \item \textbf{1687}: Newton's \textit{Principia}, universal gravitation explains all three
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Galileo's Observations of the Moon}
  \begin{columns}[c]
    \column{0.35\textwidth}
    \centering
    \includegraphics[height=0.65\textheight]{galileo_moon_drawings}
    \source{Galileo (1610)}
...
\begin{frame}{The Space Age Begins}
  \begin{columns}[c]
    ...
    \source{Mariner~4, NASA/JPL (1965)}
...
\begin{frame}{The Exoplanet Revolution}
  \begin{itemize}
    \item \textbf{1992}: Wolszczan \& Frail detect planets orbiting a \textbf{pulsar}\\
          $\rightarrow$ First confirmed exoplanets
    \item \textbf{1995}: Mayor \& Queloz detect \textbf{51 Pegasi b}\\
...
\begin{frame}{Exoplanet Discoveries Over Time}
  \begin{columns}[c]
    \column{0.50\textwidth}
    \centering
    \includegraphics[width=\textwidth]{exo_disc_year}
    \source{NASA Exoplanet Archive (2025)}

    \column{0.50\textwidth}
    \centering
    \includegraphics[width=\textwidth]{exo_disc_cumul}
    \source{NASA Exoplanet Archive (2025)}
  \end{columns}
  \vskip4pt
  {\small \textbf{Left:} Annual discoveries by detection method. \textbf{Right:} Cumulative total.\\
  The transit method has dominated since Kepler (2009); we will revisit this in Lecture~13.}
\end{frame}
...
\begin{frame}{Moons as Natural Experiments}
  \centering
  \includegraphics[width=0.60\textwidth]{galilean_moons}
  \source{NASA/JPL/DLR; Io, Europa, Ganymede, Callisto (to scale)}
  \vskip2pt
  \begin{columns}[T]
    \column{0.48\textwidth}
    \begin{itemize}\itemsep1pt
      \item All orbit Jupiter, differ in tidal heating
      \item \textbf{Io}: most volcanically active body
      \item \textbf{Europa}: subsurface ocean
    \end{itemize}
    \column{0.48\textwidth}
    \begin{itemize}\itemsep1pt
      \item \textbf{Ganymede}: ocean + magnetic field
      \item \textbf{Callisto}: ancient, undifferentiated
      \item Same system $\rightarrow$ vastly different outcomes
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Recent \& Upcoming Missions}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item \textbf{Dragonfly} (launch $\sim$2028, arrival $\sim$2034): drone on Titan
    \end{itemize}
...
\begin{frame}{Surface Exploration: Present \& Future}
  ...
  \textbf{Right:} Dragonfly will explore Titan's organic-rich surface (launch $\sim$2028, arrival $\sim$2034).}
\end{frame}
...
\begin{frame}{Summary}
  \begin{enumerate}
    ...
    \item Comparative planetology, which treats worlds as natural experiments, is our most powerful tool
...
\begin{frame}[plain,noframenumbering]
  ...
  \begin{center}
    ...
    \vskip20pt
    {\color{ipsLightFill}\normalsize Next: \textbf{Lecture~2. Planet Formation \& Orbital Dynamics}}
    \vskip16pt
