...
\begin{frame}
  \ipshero{chondrule_cai_timeline}{(a) CAIs at 4567.30 Myr define $t = 0$ and individual chondrules date from $t = 0$ to $2.6$ Myr; (b) a chondrule heating event from a 1700 K peak, cooling at 100 and 1000 K per hour. Course figure.}
\end{frame}
...
\begin{frame}
  \ipsherokey[0.65]{pallasite}{Esquel pallasite: yellow-green olivine in silvery Fe-Ni metal. Credit: UCLA Meteorite Gallery, CC BY-SA 4.0 (Wikimedia Commons).}{\textbf{Likely a sample of a planetesimal's core-mantle boundary:} molten metal mixed into the olivine of the mantle, possibly during a giant impact.}
\end{frame}
...
\begin{frame}
  \ipshero[0.76]{pb_pb_dating}{Pb-Pb isochrons for CAI 22E (Efremovka) and chondrules in the inverse form: the intercept of the mineral-fraction line at zero $^{204}$Pb gives the radiogenic $^{207}$Pb/$^{206}$Pb and so the age $t$, independent of the initial Pb. From Connelly et al.\ (2012).}
\end{frame}
...
\begin{frame}
  \ipsherokey[0.65]{demeo_carry_belt_inclination}{Asteroid orbits in inclination against heliocentric distance: Hungarias, the main belt at 2.1--3.3 AU, Hildas, and the Trojans at 5.2 AU. DeMeo \& Carry (2014).}{A total mass of $\sim 4 \times 10^{-4}\,\Mearth$ (Ceres $\approx 40\%$); the vertical gaps are the Kirkwood resonances with Jupiter.}
\end{frame}

\begin{frame}
  \ipsherokey{granvik_kirkwood_resonances}{Orbits of the near-Earth objects that leave the main belt through the 3:1 (2.50 AU), 5:2 (2.82) and 2:1 (3.27) resonances with Jupiter. Granvik et al.\ (2018).}{Periodic Jovian kicks add coherently and pump eccentricities to planet-crossing in $10^4$--$10^6$~yr: the gaps are not empty but leaky, the dynamical sinks that feed the near-Earth population.}
\end{frame}

\begin{frame}
  \ipsherokey[0.65]{asteroid_families_nesvorny}{Asteroid families as clusters in proper inclination against proper semimajor axis: Themis, Eos, Koronis, Eunomia, Vesta, Flora. Nesvorn\'y et al.\ (2015).}{Each family is the debris of a single disruption event; its spread gives the original parent body's size and age.}
\end{frame}

\begin{frame}
  \ipsherokey[0.61]{demeo_carry_belt_mass}{Mass of the main belt by spectral class against semimajor axis. DeMeo \& Carry (2014).}{\textbf{S-types} ($\sim 15\%$ albedo, ordinary-chondrite analogues) fill the inner belt, \textbf{C-types} ($\sim 4\%$ albedo, volatile-rich) $\sim 75\%$ of the outer belt, \textbf{M- and V-types} are cores and Vesta's basaltic crust ($2.4$~AU). Radial sorting preserves the disk's primordial volatile gradient.}
\end{frame}
...
\begin{frame}
  \ipsherokey[0.6]{oumuamua_discovery_meech}{Deep image of 1I/'Oumuamua at the centre; the stars trail because the telescope tracked the target. Credit: ESO / K.\ Meech et al.}{No detectable coma at the limit of stacked imaging, yet the outgoing trajectory shows a non-gravitational acceleration; its nature is still debated.}
\end{frame}
