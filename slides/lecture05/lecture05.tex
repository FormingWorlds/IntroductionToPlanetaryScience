...
\begin{frame}
  \centering
  \makebox[\linewidth][c]{\includegraphics[width=0.95\paperwidth, height=0.72\paperheight, keepaspectratio]{helled2014_core_accretion}}\\
  {\footnotesize Growth of a giant planet by core accretion: runaway gas accretion builds the $\mathrm{H_2}$/He envelope within the disk lifetime once envelope mass overtakes core mass. Reproduced from Helled et al.\ (2014), Fig.\ 1; simulation from Lissauer et al.\ (2009).}
\end{frame}
...
\begin{frame}
  \centering
  \makebox[\linewidth][c]{\includegraphics[width=0.95\paperwidth, height=0.72\paperheight, keepaspectratio]{outgassing_speciation_fO2}}\\
  {\footnotesize Speciation of an outgassed atmosphere as a function of melt oxygen fugacity: CO, $\mathrm{H_2}$, and $\mathrm{CH_4}$ dominate under reducing conditions, while $\mathrm{CO_2}$ and $\mathrm{H_2O}$ dominate under oxidising conditions. Course figure; model of Nicholls et al.\ (2024).}
\end{frame}
...
\begin{frame}
  \centering
  \makebox[\linewidth][c]{\includegraphics[width=0.95\paperwidth, height=0.72\paperheight, keepaspectratio]{lyons2014_oxygen_history}}\\
  {\footnotesize Earth's atmospheric oxygen through time: anoxic for its first $\sim$2~Gyr, rising abruptly at the Great Oxidation Event near 2.4~Ga, and reaching modern levels in the Phanerozoic through biological photosynthesis. Reproduced from Lyons et al.\ (2014).}
\end{frame}
...
\begin{frame}
  \centering
  \makebox[\linewidth][c]{\includegraphics[width=0.95\paperwidth, height=0.72\paperheight, keepaspectratio]{ideal_gas_density}}\\
  {\footnotesize Ideal gas law in atmospheric form: $\rho = P\mu m_u/(k_B T)$. Density scales with pressure at fixed $T$; heavier or colder gas is denser. Course figure.}
\end{frame}

\begin{frame}
  \centering
  \makebox[\linewidth][c]{\includegraphics[width=0.95\paperwidth, height=0.72\paperheight, keepaspectratio]{barometric_profile}}\\
  {\footnotesize The barometric formula $P/P_0 = e^{-z/H}$: a factor $e$ per scale height, 99\% of the column mass below $4.6\,H$; a straight line on a logarithmic axis. Course figure.}
\end{frame}
...
\begin{frame}
  \centering
  \makebox[\linewidth][c]{\includegraphics[width=0.95\paperwidth, height=0.72\paperheight, keepaspectratio]{scale_height_bar}}\\
  {\footnotesize Scale heights recomputed from $T$, $\mu$ and $g$: light gas (Jupiter) and weak gravity (Titan) give the largest $H$, Earth the smallest. Course figure.}
\end{frame}
...
\begin{frame}
  \centering
  \makebox[\linewidth][c]{\includegraphics[width=0.95\paperwidth, height=0.72\paperheight, keepaspectratio]{titan_haze_pia06160}}\\
  {\footnotesize Detached haze layers in Titan's upper atmosphere imaged at the limb by Cassini ISS: photochemical haze layers extending up to 500~km altitude absorb solar UV and heat the stratosphere. Credit: NASA/JPL/Space Science Institute, public domain.}
\end{frame}
...
\begin{frame}
  \centering
  \makebox[\linewidth][c]{\includegraphics[width=0.95\paperwidth, height=0.72\paperheight, keepaspectratio]{rayleigh_scattering}}\\
  {\footnotesize Rayleigh scattering and the colour of the sky: shorter path lengths at noon preserve blue scattered light, while longer slant paths at sunset transmit only reddened light; cross-section scales as $\lambda^{-4}$. Course figure.}
\end{frame}
...
\begin{frame}
  \centering
  \makebox[\linewidth][c]{\includegraphics[width=0.95\paperwidth, height=0.72\paperheight, keepaspectratio]{atmospheric_transmission}}\\
...
\begin{frame}
  \centering
  \makebox[\linewidth][c]{\includegraphics[width=0.95\paperwidth, height=0.72\paperheight, keepaspectratio]{radiative_convective_profile}}\\
  {\footnotesize Radiative-convective equilibrium (schematic): the radiative profile is superadiabatic near the surface, convection holds the troposphere on the dry adiabat, the tropopause is where the two gradients cross. Course figure.}
\end{frame}
...
\begin{frame}
  ...
  {\footnotesize Surface temperature and surface pressure of Venus, Earth, and Mars: planets of similar bulk composition span a factor of $10^4$ in surface pressure and 500~K in surface temperature due to greenhouse warming. Course figure; data from NASA.}
\end{frame}
...
\begin{frame}
  \centering
  \makebox[\linewidth][c]{\includegraphics[width=0.95\paperwidth, height=0.60\paperheight, keepaspectratio]{radius_valley_owen2019}}\\
  {\footnotesize Population synthesis of close-in planets after 10 Gyr of EUV-driven escape: planet radius against orbital separation (left, cores of 6.5 to 15 $\Mearth$) and against incident flux (right, colour the retained H/He mass fraction). The evaporation desert at small separations and large radii is empty, and the evaporation valley opens near 1.5 to 2 $\Rearth$: stripped cores below it, envelope-keeping planets above. Left panel after Owen \& Wu (2013), right panel after Lopez \& Fortney (2013), labels after Owen (2019).}
\end{frame}
...
\begin{frame}
  \centering
  \makebox[\linewidth][c]{\includegraphics[width=0.95\paperwidth, height=0.72\paperheight, keepaspectratio]{hot_jupiter_outflow_tripathi2015}}\\
  {\footnotesize Hydrodynamic outflow from a hot Jupiter driven by stellar EUV heating: heated gas accelerates outward into a night-side wake with Kelvin-Helmholtz shear instabilities. Reproduced from Owen (2019), Fig.\ 2; simulation from Tripathi et al.\ (2015).}
\end{frame}
...
\begin{frame}
  \centering
  \makebox[\linewidth][c]{\includegraphics[width=0.95\paperwidth, height=0.66\paperheight, keepaspectratio]{maven_ion_plume}}\\
  {\footnotesize The ion plume of Mars from MAVEN data: the most energetic ions (red) are accelerated in a plume above the planet, the bulk of the escaping ions (green) leave along the tail behind Mars. Without a global dipole field the solar wind reaches deep into the upper atmosphere, so unmagnetised Mars is exposed to this solar-wind-driven loss. Credit: NASA Scientific Visualization Studio (SVS 4370), public domain.}
\end{frame}
...
\begin{frame}
  \centering
  \makebox[\linewidth][c]{\includegraphics[width=0.95\paperwidth, height=0.72\paperheight, keepaspectratio]{johnstone2021_xuv_evolution}}\\
  {\footnotesize Stellar X-ray luminosity evolution for Sun-like stars and M dwarfs: lower-mass stars remain in the saturated high-activity regime for over 1~Gyr, driving prolonged atmospheric stripping on close-in rocky exoplanets. Reproduced from Johnstone et al.\ (2021), Fig.\ 11.}
\end{frame}
