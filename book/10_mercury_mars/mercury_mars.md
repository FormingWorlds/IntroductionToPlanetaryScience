...
The only commensurabilities at which a permanent torque can lock the spin are therefore $1{:}1$ ($p=0$), $3{:}2$ ($p=1$), $2{:}1$ ($p=2$), and so on. Which one Mercury actually settles into is set by the *strength* of each resonance, weighted by the corresponding eccentricity-dependent Hansen coefficient, the Fourier coefficient that sets how strongly a given resonance is excited by the orbital eccentricity. For the $1{:}1$ state the strength scales as a constant, while for $3{:}2$ it scales roughly as $e$ and for $2{:}1$ as $e^2$. With $e = 0.206$, the $3{:}2$ resonance is wide enough that capture probability dominates the others, and dissipation can no longer drag the spin all the way down to $1{:}1$ {cite:p}`CorreiaLaskar2004`.
...
3. **One or more giant impacts.** A high-energy collision late in accretion could have stripped most of an originally Earth-like silicate mantle from a proto-Mercury, leaving a body dominated by its metallic core. This idea has been around since the 1980s and remains the leading hypothesis. {cite:t}`Chau2018` and others have run smoothed-particle hydrodynamics simulations, a numerical technique that models a fluid as a set of interacting particles, to test specific scenarios, finding that a single hit-and-run encounter with a much larger projectile, or a sequence of moderate impacts, can in principle produce the right core mass fraction. {cite:t}`Franco2022` recently emphasised, however, that such configurations are dynamically rare in N-body integrations of the inner solar system: explaining Mercury via a single giant impact happens in well below $1\%$ of plausible histories ({numref}`fig:franco-mercury`). Multiple smaller impacts may be statistically more likely.
...
Mars is the second-smallest planet, with a mass of $0.107\,\Mearth$ and a radius of $0.532\,\Rearth = 3389\ \mathrm{km}$. Its mean density is $3.93\ \mathrm{g\ cm^{-3}}$, the lowest of the four terrestrial planets. The orbit sits near the outer edge of the conservative habitable zone of {ref}`Lecture 5 <lecture05>`, with semi-major axis $a = 1.524\ \mathrm{AU}$ and eccentricity $e = 0.0934$. Crucially, the obliquity is currently $25.19^\circ$, almost identical to Earth's $23.4^\circ$, so Mars has Earth-like seasons. The atmosphere is thin ($\sim 6\ \mathrm{mbar}$ surface pressure) and dominated by $\mathrm{CO_2}$, with a global mean surface temperature near $210\ \mathrm{K}$.
...
By the summer of 2021, the InSight team had announced three landmark results, all published together in *Science* {cite:p}`Stahler2021,Khan2021,Knapmeyer-Endrun2021`. {cite:t}`Stahler2021` used $ScS$ phases (shear waves reflected off the core-mantle boundary) from several large quakes to measure the core radius directly: $1830 \pm 40\ \mathrm{km}$, considerably *larger* than most pre-InSight thermal-evolution models had predicted, and corresponding to about $54\%$ of the planet's radius. Two 2023 reanalyses {cite:p}`Khan2023,Samuel2023` reinterpret this boundary as the top of a molten silicate layer ({cite:t}`Khan2023`: $150 \pm 15$ km thick) atop the metallic core, revising the iron-core radius downward to $\sim$1650–1675 km (see {ref}`Lecture 8 <lecture08>` for the updated picture). The fact that $ScS$ waves are detected means the outer core is liquid; if the core were solid, shear waves could partially propagate across the boundary into the core and the strong reflected $ScS$ signature would be greatly reduced. The measurement implies a mean core density of $5.7$ to $6.3\ \mathrm{g\ cm^{-3}}$, well below pure iron, requiring a significant complement of light elements (sulfur, oxygen, carbon, hydrogen) dissolved in the iron core ({numref}`fig:stahler-quakes`).
...
The youthful interior structure that InSight reveals is consistent with a separate, much older line of evidence: Mars formed and differentiated very fast. The hafnium-tungsten ($^{182}\mathrm{Hf}$-$^{182}\mathrm{W}$) chronometer applied to Martian meteorites and bulk silicate Mars, the planet's silicate portion excluding the metallic core, indicates that Mars accreted and segregated its metallic core within only $\sim 2$--$4$ Myr of CAI formation (the condensation of the calcium-aluminium-rich inclusions taken as the solar system's formation time-zero) {cite:p}`Kruijer2017Mars`. This is a remarkably short timescale, much shorter than the $\sim 30$--$100$ Myr accretion time of Earth, and it implies that **Mars is essentially a planetary embryo that never grew into a fully fledged terrestrial planet**. The small core fraction, the short dynamo lifetime, and the apparent lack of late giant impacts on Mars are all consistent with a body that finished its main accretion phase before the Solar System's terrestrial-planet region had even started to consolidate.
...
A planet in steady state would have $\mathrm{Ur} = 1$: every joule produced by $^{40}\mathrm{K}$, $^{232}\mathrm{Th}$, $^{235}\mathrm{U}$, and $^{238}\mathrm{U}$ decay would be radiated away through the surface as fast as it is generated. $\mathrm{Ur} > 1$ means the interior is heating up; $\mathrm{Ur} < 1$ means it is drawing on a finite reservoir of stored heat (either residual accretion energy or earlier radiogenic input) and cooling. For Mars the modern radiogenic budget is $H_{\mathrm{rad}} \sim 2.5 \times 10^{12}\ \mathrm{W}$ for a Wänke-Dreibus bulk composition, while the InSight-anchored thermal-evolution models of {cite:t}`Plesa2022` give an average present-day surface flux of $\sim 20\ \mathrm{mW\,m^{-2}}$, corresponding to a global heat output of $Q_{\mathrm{surf}} \approx 20 \times 10^{-3}\ \mathrm{W\,m^{-2}} \times 1.45 \times 10^{14}\ \mathrm{m^2} \approx 3 \times 10^{12}\ \mathrm{W}$. Plugging in,

$$
\mathrm{Ur}_{\mathrm{Mars}} \;\sim\; \frac{2.5 \times 10^{12}}{3 \times 10^{12}} \;\approx\; 0.8\, ,
$$

which is below unity but only modestly. Mars is currently losing heat slightly faster than radioactive decay deposits it, so the interior is cooling on a secular timescale set by its bulk thermal inertia, but at a much slower fractional rate than Earth ($\mathrm{Ur}_\oplus \approx 0.3$--$0.5$). This is consistent both with the long-extinct dynamo and with the picture of a single near-stagnant Tharsis plume rather than a vigorous convecting mantle, and it places Mars firmly on the slow-cooling track expected for a small stagnant-lid body whose lithosphere impedes efficient heat extraction from the interior.


### Mars' geological epochs: Noachian, Hesperian, Amazonian

Mars has been carved into three major geological epochs based on crater densities (older surfaces accumulate more impact craters over time), calibrated against the lunar cratering chronology of {cite:t}`Neukum2001` and refined for Mars by {cite:t}`Hartmann2001`. The most recent global compilation is the geologic map of {cite:t}`Tanaka2014` ({numref}`fig:tanaka-global` and {numref}`fig:tanaka-periods`), which underlies most modern discussions of Mars' history.
...
The **Amazonian** epoch ($\sim 3.0$ Ga to present) covers two thirds of Mars' history but represents only a small fraction of its total geological activity. Amazonian-aged surfaces are characterised by very low crater densities, sporadic volcanism (concentrated in the Tharsis region, where eruptions continued well into the last $1$ Gyr), modest aeolian (wind-driven) and periglacial (cold-climate freeze-thaw) activity, and a thin, dry atmosphere essentially indistinguishable from the modern one.
...
- **Olympus Mons**, a shield volcano whose summit reaches $\sim 21.3\ \mathrm{km}$ above the Mars datum (areoid) and $\sim 22\ \mathrm{km}$ above its surrounding plains, with a base diameter of roughly $600\ \mathrm{km}$;
...
Several escape routes from this contradiction have been proposed. One important class of solutions invokes additional reducing greenhouse gases such as $\mathrm{H_2}$ or $\mathrm{CH_4}$, supplied by volcanic outgassing or by serpentinisation, the water-rock reaction that alters iron- and magnesium-rich (ultramafic) minerals and releases hydrogen. {cite:t}`Wordsworth2017` showed that **collisionally induced absorption (CIA)** between $\mathrm{CO_2}$ and either $\mathrm{H_2}$ or $\mathrm{CH_4}$ has a much larger effect than previously appreciated. Because the $\mathrm{CO_2}$ quadrupole moment is $\sim 3\times$ that of $\mathrm{N_2}$, and CIA scales as quadrupole squared, the $\mathrm{CO_2}$-$\mathrm{H_2}$ and $\mathrm{CO_2}$-$\mathrm{CH_4}$ pairs absorb roughly an order of magnitude more strongly in the critical $250$--$500\ \mathrm{cm^{-1}}$ window than the $\mathrm{N_2}$-substitute estimates used in earlier work. With these stronger CIAs, modest concentrations of either gas (a few percent by volume, or both combined: e.g. $3.5\%\ \mathrm{H_2} + 3.5\%\ \mathrm{CH_4}$ in a $1.5\ \mathrm{bar}$ $\mathrm{CO_2}$ atmosphere) can raise mean surface temperatures above the freezing point. This allows a warm or warm-and-episodic early Mars without unphysical $\mathrm{CO_2}$ pressures. The four end-member climate regimes (warm-and-wet, cold-and-wet, cold-and-dry, warm-and-dry) and their geomorphological signatures are summarised in the {cite:t}`Wordsworth2016` phase diagram ({numref}`fig:wordsworth-phase`).
...
Idealised two-dimensional phase diagram for the long-term state of early Mars from {cite:t}`Wordsworth2016`, with steady-state mean surface temperature on the horizontal axis ($T_{\mathrm{surf}} \approx 280$ K marker shown) and total surface $\mathrm{H_2O}$ inventory on the vertical axis ($\approx 200$ m global equivalent layer marker shown). Each quadrant corresponds to a distinct end-member regime: warm-and-wet (with a northern ocean), cold-and-wet (with extensive wet-based glaciation), cold-and-dry (the "icy highlands" scenario), and warm-and-dry. Schematic cross-sections in each quadrant show the resulting north-south distribution of surface ice (white-grey) and liquid water (blue) over the dichotomy. The geomorphological constraints favour the cold-and-dry regime as the time-averaged state, with episodic excursions to wetter conditions.
```
...
A third element is the dramatic shift in the spatial distribution of fluvial features from the Noachian to the Hesperian, mapped by {cite:t}`KiteCarter2022` ({numref}`fig:kite-distribution`). In the Noachian, fluvial activity is widespread across the southern highlands. In the late Hesperian and Amazonian, what little fluvial activity remains is concentrated in mid-latitude alluvial fans, fan-shaped sediment deposits where a channel loses gradient and slows, and deltas. {cite:t}`KiteCarter2022` argued that this latitudinal migration encodes a fundamental change in Mars' greenhouse mechanism between the two epochs: an early greenhouse sustained over the whole planet, followed by a late one that worked only seasonally and only at certain latitudes ({numref}`fig:kite-schematic`).
...
Although Mars has no global magnetic field today, the *Mars Global Surveyor* magnetometer discovered in 1999 that large parts of the southern highlands carry intense **crustal magnetic anomalies** {cite:p}`Acuna1999`. These are remanent fields, frozen into the Noachian crust when basaltic rocks cooled through their Curie temperatures, the temperature below which a mineral can retain a permanent magnetisation, in the presence of an ancient core dynamo. The magnetic anomalies are spectacularly strong (locally exceeding $\sim 1500\ \mathrm{nT}$ at $\sim 100\ \mathrm{km}$ altitude during the *MGS* aerobraking phase, an order of magnitude stronger than the strongest crustal anomalies on Earth), confirming that the Martian dynamo, while it operated, produced a vigorous magnetic field ({numref}`fig:acuna-map` and {numref}`fig:acuna-dipoles`).
...
The spatial pattern is the key to dating the dynamo. The Hellas, Argyre, and Isidis impact basins, each of which formed by very large impacts in the Early Noachian around $4.0$-$4.1$ Ga (the Hellas-forming impact is the stratigraphic marker for the start of the Noachian), are essentially *demagnetised* relative to the surrounding terrain. The interpretation is that these impacts heated the underlying crust above the magnetite Curie temperature ($\sim 850\ \mathrm{K}$) and erased its remanent magnetisation; when the crust cooled back down, there was no longer an active dynamo to record. Bracketing the basin ages places the death of the Martian dynamo around $4.1$--$3.9\ \mathrm{Ga}$ in the original {cite:t}`Acuna1999` analysis.

This picture has been refined over time. {cite:t}`Mittelholz2020` analysed *MAVEN* magnetometer data from low-altitude passes and detected significant remanent fields associated with both an early ($\sim 4.5\ \mathrm{Ga}$, dichotomy-related) phase and a much later ($\sim 3.7\ \mathrm{Ga}$, Lucus Planum lava flow) phase, suggesting that the Martian dynamo was either longer-lived than the basin-demagnetisation estimate or operated in distinct episodes punctuated by quiet intervals. They also propose that the apparent demagnetisation of the giant basins may reflect *excavation* of the strongly magnetised upper crust (exposing less-magnetic, more-ultramafic deeper material) rather than purely thermal erasure. {cite:t}`Steele2024` recently proposed an alternative: a *reversing* dynamo, in which the geometry of the field was changing on timescales short compared to basin cooling times, would smear out the recorded direction across the basin floor. Both findings push the death of the Martian dynamo somewhat later, possibly into the early Hesperian, and reduce the temporal gap between the loss of the dynamo and the loss of the atmosphere.
...
The **MSR programme itself** is going through difficulties. The original NASA-ESA architecture targeted return of the cached samples in the early 2030s. In 2023, an Independent Review Board concluded that the reference mission was too expensive and too slow {cite:p}`NASAESAMSR2023`, and in 2024 NASA initiated a major architecture-review process in response. As of early 2026 the architecture, the schedule, and the total cost are all uncertain, with industry studies and competing concepts under evaluation. The scientific imperative remains, but the path from a *Perseverance* sample tube on Mars to a clean-room laboratory on Earth has become much more uncertain than it appeared a few years ago.
...
- **Volatile inventory** at formation depends on heliocentric distance through the ice line and through the timing of volatile delivery from outer-disc bodies (compare with {ref}`Lecture 2 <lecture02>`).
...
Synthesising the lessons of {ref}`Lecture 9 <lecture09>` and this lecture, we can identify at least four largely independent ingredients that a rocky planet appears to need for long-term surface habitability:
...
We will return to this list explicitly in {ref}`Lecture 14 <lecture14>` when we put together the synthesis of habitability across the diverse rocky-planet population that the next decade of observation will reveal.
