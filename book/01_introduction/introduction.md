...
3. **Are we alone?** The **James Webb Space Telescope** (JWST) is currently attempting to detect and characterise atmospheres on rocky exoplanets, including the TRAPPIST-1 system. Within your careers, this question may become answerable, but only if we understand what makes a planet habitable in the first place.
...
<!-- Generated by scripts/figures/L01_introduction/fig_exoplanet_mass_period.py -->
...
**Key properties of the eight solar system planets.** Data from {cite:p}`NASAFactSheet`. Radii are equatorial; the densities are bulk values based on the volumetric mean radius, so for the oblate giant planets $\rho$ computed from the tabulated $M$ and $R$ comes out up to $10\%$ low.
...
<!-- Generated by scripts/figures/L01_introduction/fig_density_vs_distance.py -->
...
<!-- Generated by scripts/figures/L01_introduction/fig_mass_vs_distance.py -->
...
{numref}`fig:board-sketch-l01` shows the setup: the star at the centre, the planet on a circle of radius $r$, its velocity tangent to the orbit and gravity pointing inward.

<!-- Generated by scripts/figures/board_sketches/render_board_sketches.py -->
```{figure} figures/board_sketch_l01.avif
:name: fig:board-sketch-l01
:width: 45%
:align: center

Setup sketch of the board derivation of the solar mass: star $M_*$ at the centre, planet $M_p$ on a circular orbit of radius $r$ with velocity $v$, and the gravitational force $F_g$ that supplies the centripetal acceleration. Course-original figure.
```
...
<!-- Generated by scripts/figures/L01_introduction/fig_ss_mass_budget.py -->
...
<!-- Generated by scripts/figures/L01_introduction/fig_vem_atmospheres.py -->
...
Ground-based telescopes observe planets across the electromagnetic spectrum. Optical imaging resolves surface and atmospheric features, infrared spectroscopy probes thermal emission and atmospheric composition, and radio observations measure subsurface properties and atmospheric dynamics. Adaptive optics on large telescopes (e.g., VLT, Keck) achieve angular resolutions approaching those of space telescopes. ESO's Very Large Telescope ({numref}`fig:vlt-paranal`), four 8.2 m unit telescopes on Cerro Paranal in the Atacama Desert, is one of the workhorses of ground-based solar-system astronomy: SPHERE-ZIMPOL has resolved disk-integrated shapes of large main-belt asteroids, NACO and SINFONI have tracked the seasonal hazes of Titan and the storms of Saturn and Jupiter, and the VLT supported the Rosetta mission with high-cadence monitoring of comet 67P/Churyumov-Gerasimenko on approach to perihelion.

```{figure} figures/vlt_paranal.avif
:name: fig:vlt-paranal
:width: 700px
:align: center
...
Spacecraft provide the most detailed planetary data. NASA's currently active and planned planetary missions are summarised in {numref}`fig:nasa-planetary-fleet`, ranging from the Voyagers in interstellar space and Juno at Jupiter to the New Frontiers (OSIRIS-APEX, Dragonfly), Discovery (Lucy, Psyche, DAVINCI, VERITAS), and Mars program assets (Perseverance + Ingenuity, Curiosity, MAVEN, Mars Reconnaissance Orbiter, the InSight legacy data set). The European counterpart ({numref}`fig:esa-planetary-fleet`) has been built up over four decades from Giotto's 1986 Halley flyby through current missions (BepiColombo at Mercury, Mars Express, the Jupiter Icy Moons Explorer JUICE en route to the Jupiter system) and into the planned EnVision Venus orbiter and the Comet Interceptor that will wait at L2 for a target.

```{figure} figures/nasa_planetary_fleet.avif
:name: fig:nasa-planetary-fleet
:width: 750px
:align: center
...
JWST F1500W secondary-eclipse measurement of TRAPPIST-1 b with the Mid-Infrared Instrument (MIRI): the first thermal measurement of a rocky exoplanet around an M dwarf. The horizontal axis is wavelength in microns; the vertical axis is the planet's spectral flux density in microjanskys (μJy). The two black points with error bars are JWST MIRI photometric measurements: the F1280W band centred on 12.8 μm (labelled *Expected*, the value the planet would emit if it followed the measured 15-μm dayside blackbody) and the F1500W band centred on 15 μm (labelled *Measured*, the secondary-eclipse depth that gives the actual planet emission at that wavelength). Five model curves are overplotted: three blackbody predictions (blue: $503 \pm 27$ K dayside brightness temperature; orange: 400 K equilibrium temperature with full heat redistribution; green: 508 K bare-rock prediction for zero heat redistribution and zero albedo) and two thick-atmosphere models (cyan: a 93-bar $\mathrm{CO_2}$-dominated atmosphere; magenta: a 10-bar $\mathrm{O_2}+\mathrm{CO_2}$ atmosphere). $\mathrm{CO_2}$ absorbs strongly near 15 μm, so the atmospheric models predict sharply suppressed flux there. The F1500W point sits on the bare-rock blackbody and well above the two atmospheric models, ruling out thick $\mathrm{CO_2}$-rich atmospheres on TRAPPIST-1 b and indicating that the planet retains little or no thick atmosphere. The atmospheric retention of M-dwarf rocky planets is the central question this kind of measurement is built to answer, treated in detail in {ref}`Lecture 13 <lecture13>`. Reproduced from {cite:t}`Greene2023`.
```

Closer to home, NASA's **OSIRIS-REx** mission returned samples from the carbon-rich asteroid Bennu in September 2023, revealing hydrated minerals and organic compounds that illuminate the primordial building blocks of planets ({ref}`Lecture 12 <lecture12>`). The moment of sample acquisition is shown in {numref}`fig:osiris-rex-tag`: the spacecraft's TAGSAM head (Touch-and-Go Sample Acquisition Mechanism) pressed into Bennu's surface for six seconds, fired a burst of nitrogen gas to mobilise the regolith, and then backed away.
...
The Mars exploration programme continues with the **Perseverance** rover caching samples in Jezero crater for return to Earth {cite:p}`Farley2022` (subject to the ongoing rebaselining of the Mars Sample Return architecture, {ref}`Lecture 10 <lecture10>`), and NASA's **Dragonfly** rotorcraft, a nuclear-powered drone that will explore Titan's surface chemistry, is planned for launch in 2028 and arrival in 2034 ({ref}`Lecture 11 <lecture11>`, {ref}`Lecture 14 <lecture14>`).

## The three central questions of this course

The bodies, missions, and measurements introduced in this lecture set up three driving questions that organise the remaining thirteen lectures. The course is structured so that each question is answered piece by piece, returning to the same set of objects, but viewed through a sharper instrument each time.

**1. How do planetary systems form?** The mass concentration in Jupiter and Saturn ({numref}`fig:ss-mass-budget`), the dust gaps imaged by ALMA in HL Tau ({numref}`fig:hl-tau-alma`), and the small-planet radius valley in {numref}`fig:radius-gap` are all clues about a process we cannot watch in real time: how a collapsing molecular cloud becomes a star surrounded by a disk, how dust grows by ten orders of magnitude in size into planetesimals and protoplanets, and how the resulting bodies are sorted by gas drag, gravitational scattering, and resonances into the orbital architecture we see today. {ref}`Lecture 2 <lecture02>` introduces the disk picture and the orbital dynamics; {ref}`Lecture 3 <lecture03>` follows the energy budget of accretion and short-lived radionuclides; {ref}`Lecture 12 <lecture12>` returns to the small bodies (asteroids, comets, Kuiper-belt objects) that preserve the formation epoch in their compositions.

**2. What makes a planet habitable?** Venus, Earth, and Mars formed from broadly the same building blocks at broadly similar heliocentric distances, and yet they span four orders of magnitude in surface pressure and 500 K in surface temperature ({numref}`fig:vem-atmospheres`). The reasons are coupled rather than independent: planetary interiors and dynamos ({ref}`Lecture 4 <lecture04>`, {ref}`Lecture 8 <lecture08>`) set the rate at which heat and volatiles leak out of the deep planet; atmospheres and climate ({ref}`Lecture 5 <lecture05>`, {ref}`Lecture 6 <lecture06>`) determine surface temperature, the rate of escape to space, and the chemistry available at the surface; surface processes ({ref}`Lecture 7 <lecture07>`) close the cycle by returning volatiles to the interior. The case studies in {ref}`Lecture 9 <lecture09>` (Earth and Venus) and {ref}`Lecture 10 <lecture10>` (Mercury and Mars) put numbers on each ingredient and explain why the three inner planets diverged.

**3. Does life exist beyond Earth?** This is the question that motivates much of the current mission queue: JWST's atmospheric spectra of rocky M-dwarf planets, exemplified by {numref}`fig:trappist1b-jwst`; JUICE and Europa Clipper at the icy moons; Dragonfly at Titan; the Extremely Large Telescope (ELT), Giant Magellan Telescope (GMT), and Thirty Meter Telescope (TMT) for direct imaging in the 2030s. {ref}`Lecture 11 <lecture11>` covers the giant planets and the ocean worlds in their moon systems. {ref}`Lecture 13 <lecture13>` places the solar system inside the exoplanet population and revisits the habitable zone. {ref}`Lecture 14 <lecture14>` closes the course on biosignatures, the Drake equation, and what the search programmes of the next two decades can and cannot answer.

## Looking ahead to Lecture 2

This lecture has been a panoramic tour: the bodies, the history, the missions, and the broad outline of the discipline. The next lecture begins the actual physics. {ref}`Lecture 2 <lecture02>` walks back to the very beginning of the story, to the molecular cloud, and follows the collapse step by step: how angular momentum conservation produces a rotating disk rather than a spherical infall, how the disk's temperature gradient sets the snow line that separates rocky from icy material, how dust grains in the disk grow by coagulation, settling, and the streaming instability to make planetesimals, and how Kepler's laws, the vis-viva equation, and orbital resonances govern the subsequent dynamics. The history of how the solar system came to look the way it does begins not with the planets but with the disk that made them.
