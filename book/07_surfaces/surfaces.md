...
{numref}`fig:impact-stages` sketches the three stages in cross-section.

<!-- Generated by scripts/figures/L07_surfaces/fig_impact_stages.py -->
```{figure} figures/impact_stages.avif
:name: fig:impact-stages
:width: 100%
:align: center

The three stages of impact crater formation in schematic cross-section. (a) Contact and compression: the projectile arrives at $10$ to $70$ km s$^{-1}$, and shock waves (dashed red) propagate into both the target and the projectile, compressing the material to pressures of order $100$ GPa and vaporising and melting the contact zone (orange). (b) Excavation: the shock front expands hemispherically, and the rarefaction wave behind it (dotted blue) accelerates material up and out along ballistic paths, opening a transient cavity with a depth of about one third of its diameter and depositing the ejecta blanket around the rim. (c) Modification: in a small crater the walls slump inward and leave a simple bowl; in a large crater the floor rebounds into a central peak and the walls collapse in terraces around a flat floor, producing a complex crater. Not to scale. Course-original figure.
```
...
{numref}`fig:board-sketch-l07` shows the quantities that enter the scaling: the impact energy $E$ arrives from above, the target has density $\rho$ and gravity $g$, and the crater has diameter $D$.

<!-- Generated by scripts/figures/board_sketches/render_board_sketches.py -->
```{figure} figures/board_sketch_l07.avif
:name: fig:board-sketch-l07
:width: 45%
:align: center

Setup sketch of the board derivation of the crater scaling law: an impactor with kinetic energy $E$ strikes a target of density $\rho$ under surface gravity $g$ and excavates a bowl of diameter $D$. Course-original figure.
```
...
## Crater morphology and chronology
...
{numref}`fig:effusive-explosive` contrasts the two eruption styles.

<!-- Generated by scripts/figures/L07_surfaces/fig_effusive_explosive.py -->
```{figure} figures/effusive_explosive.avif
:name: fig:effusive-explosive
:width: 100%
:align: center

Effusive and explosive volcanism in schematic cross-section. (a) Low-viscosity basaltic magma (about 50% SiO$_2$) flows easily: dissolved H$_2$O and CO$_2$ escape gradually, eruptions are effusive, and the products are broad shield volcanoes and thin lava sheets fed from fissures, the dominant style on the Moon, Mars and Io. (b) High-viscosity silicic magma (more than 65% SiO$_2$) traps its volatiles under a viscous plug until the pressure exceeds the strength of the magma, and the eruption is explosive, as at Mount St. Helens and Krakatoa; repeated eruptions build steep stratovolcanoes from alternating layers of lava and ash. Course-original figure.
```

### Volcanic landforms across the solar system

**Olympus Mons (Mars)** is the largest volcano in the solar system: a shield volcano with a base diameter of $\sim$600 km and a summit elevation of $\sim$21.2 km above the Mars datum (2.4 times the height of Mount Everest above sea level). It grows so large because Mars lacks plate tectonics: the volcanic hotspot remains stationary beneath the **lithosphere**, the rigid outer shell of crust and uppermost mantle, for billions of years, piling up lava in one location. On Earth, plate motion carries the crust over the hotspot, creating chains of smaller volcanoes (e.g., the Hawaiian Islands) rather than a single massive edifice {cite:p}`dePaterLissauer2010`. Surface gravity sets a second, independent limit on how tall a volcano can grow: the maximum height a crust of a given strength can support scales inversely with $g$, and with $g_{\mathrm{Mars}} \approx 0.38\,g_\oplus$ the Martian crust can hold an edifice roughly 2.6 times taller than an equally strong crust on Earth. The two effects act together: the fixed plume supplies one edifice for billions of years, and the low gravity raises the ceiling that edifice can reach.
...
Full-disk view of Jupiter's moon Io from NASA's Galileo spacecraft (PIA00583), revealing a surface dominated by sulfur and silicate volcanism. Yellow, white, and pale-green regions are sulfur and sulfur-dioxide deposits; darker patches mark recent silicate lava flows and active volcanic centres (a few of which have visible halos of fresh pyroclastic deposits). Credit: NASA/JPL-Caltech/University of Arizona, public domain.
```
...
<!-- Generated by scripts/figures/L07_surfaces/fig_olympus_mons_size_comparison.py -->
...
Synthetic aperture radar (SAR) transmits microwave pulses and images a surface regardless of illumination or cloud cover. Magellan (1990–1994) mapped 98% of Venus's surface at $\sim$100 m resolution through the permanent cloud deck ({numref}`fig:venus-magellan`), and almost all of Venus's surface geology still rests on that one dataset ({ref}`Lecture 9 <lecture09>`); Cassini's radar found Titan's methane lakes and seas through its organic haze.
...
{numref}`fig:space-weathering` summarises the agents of space weathering and their spectral result.

<!-- Generated by scripts/figures/L07_surfaces/fig_space_weathering.py -->
```{figure} figures/space_weathering.avif
:name: fig:space-weathering
:width: 100%
:align: center

Space weathering of an airless surface. (a) Schematic cross-section of a regolith grain: implanted solar wind ions (H$^+$, He$^{2+}$) and micrometeorites arriving at $10$ to $70$ km s$^{-1}$ convert the top $\sim 100$ nm of the grain into an amorphous rim that holds nanophase metallic iron blobs of a few to several hundred nm; micrometeorite impacts also produce melt splash and vapour, and galactic cosmic rays penetrate deeper into the grain. (b) Qualitative reflectance spectra: a mature, space-weathered surface is darker and redder than a fresh surface and shows a shallower $1$ $\mu$m absorption band. No numerical reflectance values are implied. Course-original figure.
```
...
Jupiter's moon **Europa** ($R \approx 1561$ km) holds a global ocean $\sim$100 km deep beneath an ice shell $\sim$15–25 km thick, maintained by tidal heating in the Laplace resonance with Io and Ganymede {cite:p}`NimmoPappalardo2016`. Its surface is crossed by **lineae**, cracks filled from below, and broken into **chaos terrain** where the shell fractured, rotated, and refroze ({numref}`fig:europa-chaos`); the near-absence of craters gives a surface age of only $\sim$40–90 Myr.
