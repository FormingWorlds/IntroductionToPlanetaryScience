...
{numref}`fig:board-sketch-l09` shows the column that the three steps below work through: the surface at $T_s$, the level where the infrared optical depth reaches $\tau \sim 1$, and the outgoing flux $\sigma T_{\mathrm{phot}}^4$ from that level.

<!-- Generated by scripts/figures/board_sketches/render_board_sketches.py -->
```{figure} figures/board_sketch_l09.avif
:name: fig:board-sketch-l09
:width: 45%
:align: center

Setup sketch of the board derivation of the Simpson-Nakajima limit: a saturated water-vapour column above a surface at temperature $T_s$, the photosphere level where the infrared optical depth from the top reaches $\tau \sim 1$ at temperature $T_{\mathrm{phot}}$, and the outgoing longwave flux $\sigma T_{\mathrm{phot}}^4$ that leaves from there. Course-original figure.
```
...
{numref}`fig:photosphere-profiles` shows this for four surface temperatures.

<!-- Generated by scripts/figures/L09_earth_venus/fig_photosphere_profiles.py -->
```{figure} figures/photosphere_profiles.avif
:name: fig:photosphere-profiles
:width: 100%
:align: center

The infrared photosphere of a saturated steam atmosphere for surface temperatures of $290$, $320$, $350$ and $400$ K, in the grey model of this derivation ($\kappa = 0.05$ m$^2$/kg, $g = 10$ m/s$^2$, $T_{\mathrm{ref}} = 273$ K).
**Left:** temperature against altitude. A saturated pure-steam column in hydrostatic balance follows $z = (L/g)\ln(T_s/T)$, a lapse rate of only $1.2$ to $1.4$ K/km because latent heat dominates the energy budget of the column. The photosphere (circles), where $p = g/\kappa = 200$ Pa, rises from $29$ to $109$ km, but its temperature stays at $T_{\mathrm{phot}} = 258$ K. The inset shows the consequence: the emitted flux $\sigma T_{\mathrm{phot}}^4 = 253$ W/m$^2$ is the same for every surface temperature.
**Right:** pressure against temperature. Every column lies on the one Clausius-Clapeyron saturation curve, from its surface (squares) upward; the photosphere pressure is a horizontal line, and the two cross at one point.
The constant latent heat overestimates the saturation pressure by about $23\%$ at the boiling point, so the surface pressures are model values; a full radiative-transfer treatment raises the flux limit to $280$ to $310$ W/m$^2$ (Step 3). Course-original figure.
```
...
{numref}`fig:venus-activity-evidence` places the three lines of evidence on one timeline.

<!-- Generated by scripts/figures/L09_earth_venus/fig_venus_activity_evidence.py -->
```{figure} figures/venus_activity_evidence.avif
:name: fig:venus-activity-evidence
:width: 100%
:align: center
