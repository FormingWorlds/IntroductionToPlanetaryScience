...
{numref}`fig:radial-velocity-signal` compares these semi-amplitudes with the instrument precisions.

<!-- Generated by scripts/figures/L13_exoplanets/fig_radial_velocity_signal.py -->
```{figure} figures/radial_velocity_signal.avif
:name: fig:radial-velocity-signal
:width: 100%
:align: center
...
```{figure} figures/board_sketch_l13.avif
:name: fig:board-sketch-l13
:width: 45%
:align: center
...
{numref}`fig:astrometric-signal` shows the amplitude against distance for the two analogues.

<!-- Generated by scripts/figures/L13_exoplanets/fig_astrometric_signal.py -->
```{figure} figures/astrometric_signal.avif
:name: fig:astrometric-signal
:width: 100%
:align: center

Astrometric reflex amplitude $\alpha = (m_p/M_\star)(a_p/d)$ against distance for a Jupiter analogue at 5.2 AU and an Earth analogue at 1 AU around a Sun-like star ($\alpha$ in arcseconds for $a_p$ in AU and $d$ in pc). At 10 pc the Jupiter analogue gives about 500 microarcseconds, half a milliarcsecond, and the Earth analogue 0.3 microarcseconds; the dashed line is Gaia's final precision of about 10 microarcseconds for bright stars, the regime in which Jupiter analogues become accessible. Course-original figure.
```
...
{numref}`fig:microlensing-lightcurve` shows the light curve of such an event.

<!-- Generated by scripts/figures/L13_exoplanets/fig_microlensing_lightcurve.py -->
```{figure} figures/microlensing_lightcurve.avif
:name: fig:microlensing-lightcurve
:width: 100%
:align: center

A microlensing event with a planet: the magnification of a background source as the lens star passes in front of it (point-lens curve with an impact parameter of 0.3 Einstein radii and an Einstein crossing time of 15 days), with the short spike that a planet around the lens adds. The spike lasts hours to days, and its amplitude and timing give the planet's mass and projected separation in units of the Einstein ring radius. Course-original figure.
```
...
{numref}`fig:transit-timing-variations` shows the timing signal and the three methods.

<!-- Generated by scripts/figures/L13_exoplanets/fig_transit_timing_variations.py -->
```{figure} figures/transit_timing_variations.avif
:name: fig:transit-timing-variations
:width: 100%
:align: center

(a) Transit timing variations: the observed minus calculated transit times of a planet perturbed by a companion oscillate about the constant-period prediction (a 5 minute amplitude over 12 transits, schematic); the amplitude and period of the variation encode the companion's mass. (b) The three timing methods: transit timing variations give dynamical masses (TRAPPIST-1), pulsar timing found the first exoplanets (1992), and eclipse timing in binaries reveals circumbinary planets (Kepler-16 b, 2011). Course-original figure.
```
