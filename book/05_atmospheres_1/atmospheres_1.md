...
<!-- Generated by scripts/figures/L05_atmospheres_1/fig_fulton_radius_valley.py -->
...
These results reflect a central theme of atmospheric science: a planet's atmospheric inventory is set by the balance between escape and resupply. Escape removes gas at a rate governed by the planet's mass and temperature (the Jeans escape criterion derived in this lecture) and by the intensity of stellar radiation and wind, while volcanic outgassing returns gas from the interior. That outgassing draws on the planet's interior heat and wanes as the interior cools ({ref}`Lecture 3 <lecture03>`), so how much atmosphere a planet retains over its history depends on its interior thermal evolution, not only on its present-day escape rate.
