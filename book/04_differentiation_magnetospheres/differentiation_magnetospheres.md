...
Empirically, partition coefficients follow a Nernst-type regression of the form:
...
The **oxygen fugacity** $f_{\mathrm{O_2}}$ adds another dimension. During Earth's accretion, $f_{\mathrm{O_2}}$ is thought to have evolved from roughly 4–5 log-units below the iron-wüstite (IW) buffer to its present mantle value near IW–2. Increasing $f_{\mathrm{O_2}}$ oxidises iron from the metal to the silicate, shifts the metal–silicate equilibrium in favour of silicate for most transition metals, and therefore reduces $D^\mathrm{met/sil}$. Self-consistent **continuous accretion models** solve for $(P, T, f_{\mathrm{O_2}})$ as a function of accreted mass and produce the observed present-day siderophile depletions as a natural outcome of a deep, progressively oxidising magma ocean rather than a single late equilibration event {cite:p}`Rubie2015`.
...
Structure of the lunar magma ocean at the end of solidification, from {cite:t}`ElkinsTanton2012`. The outer $\sim 40$–$50$ km is an anorthositic crust formed by flotation of plagioclase on top of the crystallising magma ocean (white layer, top). Beneath it a thin **urKREEP** layer (yellow, KREEP = potassium, rare earth elements, and phosphorus) represents the last $\sim 1\%$ of the original magma ocean to solidify and is concentrated in incompatible trace elements, including the heat-producing isotopes $^{40}\mathrm{K}$, $^{235,238}\mathrm{U}$, and $^{232}\mathrm{Th}$. Beneath the urKREEP layer, dense ilmenite-rich cumulates (dark green) formed near the end of crystallisation are gravitationally unstable and sank toward the core, producing the "lunar mantle overturn" that dominated lunar tectonics in the first $\sim 500$ Myr after crystallisation. The Procellarum KREEP Terrane on the lunar nearside preserves the highest surface abundance of KREEP-bearing rocks and is inferred to be the surface expression of the deep urKREEP reservoir. Reproduced from {cite:t}`ElkinsTanton2012`.
```
...
Numerical dynamo simulations establish a robust empirical scaling between the surface dipole field $B_\mathrm{dip}$, the convective buoyancy flux $F_q$ driving the flow, and the thickness $D$ of the convecting shell {cite:p}`ChristensenAubert2006,OlsonChristensen2006`:
...
The key points are: (i) Mercury's dynamo shell is relatively thin because its core is nearly as large as the planet itself, leaving only a shallow convecting layer, whereas Ganymede's dynamo shell is similarly thin but sits in a much smaller absolute core; (ii) both bodies nonetheless satisfy the magnetic-Reynolds criterion ($\mathrm{Rm} \gtrsim 10$) and the scaling of Eq. {eq}`eq:dynamo-scaling`, because the product $F_q \cdot D$ is what matters, not $D$ alone. Mercury's weaker surface field reflects primarily the deeper radial attenuation from the dynamo region to the surface ($B_\mathrm{surf} \propto (R_\mathrm{core}/R_\mathrm{planet})^3 \cdot B_\mathrm{dynamo}$), not a weaker dynamo per se. This is the same argument that explains why Jupiter has a surface field $20{,}000$ times Earth's despite not having $20{,}000$ times more vigorous convection: Jupiter's dynamo operates far closer to the surface in fractional-radius terms.
