...
Other meteorites are **differentiated meteorites** that melted and segregated into crusts and mantles (**achondrites**), metallic cores (**iron meteorites**), and mixed silicate-metal boundaries (**stony irons**).
...
Polished and acid-etched surface of an iron meteorite displaying the **Widmanstätten pattern**: an interlocking lattice of kamacite and taenite, two Fe-Ni phases. The pattern can only form if the solid metal cools at a rate slower than $\sim 100$ K per million years inside the metallic core of a parent body of asteroid size or larger. Image credit: H. Raab, CC BY-SA 3.0 (Wikimedia Commons).
```

The **Widmanstätten pattern** ({numref}`fig:l12-widmanstatten`) forms as Fe-Ni phases unmix during slow cooling ($< 100$ K per million years) in metallic cores, indicating parent bodies tens to hundreds of kilometres in radius {cite:p}`Goldstein2009`.

**Pallasites** ({numref}`fig:l12-pallasite`) are stony irons sampling the core-mantle boundary of a differentiated parent body.
...
<!-- Generated by scripts/figures/L12_small_bodies/fig_chondrule_cai_timeline.py -->
```{figure} figures/chondrule_cai_timeline.avif
:name: fig:chondrule-cai-timeline
:width: 100%
:align: center
...
```{figure} figures/board_sketch_l12.avif
:name: fig:board-sketch-l12
:width: 45%
:align: center
...
{numref}`fig:petrologic-types` lays out both scales.

<!-- Generated by scripts/figures/L12_small_bodies/fig_petrologic_types.py -->
```{figure} figures/petrologic_types.avif
:name: fig:petrologic-types
:width: 100%
:align: center

The two scales that record what a parent body did to a chondrite. Petrographic type 3 is the least altered; types 2 and 1 record increasing aqueous alteration, in which water forms clays and erases chondrules, and types 4 to 7 increasing thermal metamorphism, in which grains coarsen, compositions equilibrate and chondrule outlines vanish. The shock stage runs from S1 (unshocked) to S6 (heavily shocked, partially melted), with planar fractures, melt veins and high-pressure minerals such as ringwoodite and majorite formed at tens of GPa. Course-original figure.
```
...
{numref}`fig:oxygen-three-isotope` sketches the diagram.

<!-- Generated by scripts/figures/L12_small_bodies/fig_oxygen_three_isotope.py -->
```{figure} figures/oxygen_three_isotope.avif
:name: fig:oxygen-three-isotope
:width: 100%
:align: center

Schematic oxygen three-isotope plot, $\delta^{17}$O against $\delta^{18}$O. Mass-dependent fractionation puts every terrestrial sample, the Moon and the enstatite chondrites on the terrestrial fractionation line of slope 1/2; CAIs define a slope-1 line that records mixing with a nearly pure $^{16}$O reservoir. Ordinary chondrites plot above the terrestrial line, carbonaceous chondrites below it, CI chondrites together with the Ryugu samples, and the HED and SNC groups on their own parallel lines for Vesta and Mars. Cluster positions are schematic; the slopes and the ordering above and below the terrestrial line follow the text. Course-original figure.
```

### The NC-CC isotopic dichotomy: three competing interpretations

**Nucleosynthetic anomalies** split meteorites into non-carbonaceous (**NC**) and carbonaceous (**CC**) reservoirs that did not mix for at least the first $\sim 3$–$4$ Myr {cite:p}`Warren2011,Kruijer2017`.
...
Hf-W metal-silicate constraints show NC core formation at $\Delta t_{\text{CAI}} = 1.0$ to $2.6$ Myr ($\varepsilon^{182}\mathrm{W} = -3.37$ to $-3.20$) and CC core formation at $3.0$ to $3.6$ Myr ($\varepsilon^{182}\mathrm{W} = -3.16$ to $-3.10$). The separation of roughly $2$ Myr between the two reservoirs is the key constraint that all three interpretations must satisfy (from Figure 5 of {cite:t}`SpitzerPt2021`).
```
...
Planetesimal formation rate $\dd \Sigma_{\text{plts}} / \dd t$ across orbital distance $r$ and time $t$ after CAI formation, where outward snow-line migration produces early dry NC bodies (Reservoir I) and inward migration forms later ice-rich CC bodies (Reservoir II). This bifurcation arises from disk thermal evolution without requiring an early formed Jupiter barrier (from Figure 1 of {cite:t}`Lichtenberg2021`).
```

Three competing hypotheses explain the physical cause of this bifurcation:
...
**Asteroid families** are clusters of asteroids sharing similar **proper elements** (orbital elements averaged over secular variations), representing collisional debris from a disrupted parent body.
...
Asteroid families in the main belt revealed by hierarchical clustering, plotted in proper inclination $i_{\text{P}}$ versus proper semimajor axis $a_{\text{P}}$. The principal Hirayama families (Themis, Eos, Koronis, Eunomia, Vesta, Flora) appear as concentrations in $(a_{\text{P}}, i_{\text{P}})$ space (from Figure 1a of {cite:t}`Nesvorny2015`).
```
...
Rocky S-types dominate the inner belt inside the snow line, whereas volatile-rich C-types dominate the outer belt ({numref}`fig:l12-demeo-mass`) {cite:p}`DeMeo2014`.
...
Compositional mass distribution across the asteroid belt in $0.02$ AU bins from the Hungarias to the Trojans, showing total mass (grey) and the spectral classes; the horizontal line at $10^{18}$ kg is the limit of the 1980s surveys, and below it the modern data resolve the individual classes. Silicaceous S-types dominate the inner belt while carbonaceous C-, B-, P-, and D-types dominate the outer belt, with V-type material concentrated near $a = 2.4$ AU, reflecting the disk volatile gradient (from Figure 3 of {cite:t}`DeMeo2014`).
```

This taxonomic structure records giant-planet migration in the early solar system, which implanted carbonaceous bodies into the outer belt and reshuffled the asteroid belt {cite:p}`Walsh2011,Raymond2017` ({ref}`Lecture 2 <lecture02>`).
...
The impact rate follows a power law with exponent $-2.3$ ({numref}`fig:l12-impact-freq`); the $10$ km Chicxulub impactor that ended the Cretaceous at $66$ Ma is the largest well-dated example (cratering physics in {ref}`Lecture 7 <lecture07>`).
...
Cumulative size-frequency distribution of near-Earth objects versus absolute magnitude $H_V$ ({cite:t}`Schunova2017`, Fig. 9). The power-law distribution spans five orders of magnitude, consistent with models from {cite:t}`Bottke2002` and {cite:t}`Granvik2018`.
```

Impact rates scale inversely with size: 10 to 80 m airbursts occur every decade to few millennia (e.g., Chelyabinsk, {numref}`fig:l12-chelyabinsk`, {cite:p}`Brown2013`, and Tunguska), 140 m impacts every $\sim 30{,}000$ years, 1 km disruptions every $\sim 500{,}000$ years, and 10 km extinctions every $\sim 100$ Myr.
...
Vapour trail of the 15 February 2013 Chelyabinsk meteor, formed when a 19 m asteroid disintegrated at $\sim 30$ km altitude releasing 0.5 Mt TNT equivalent. The shock wave shattered windows and injured about 1,500 people (image credit: Alex Alishevskikh, CC BY-SA 2.0).
```

In 2022, NASA's DART mission demonstrated **planetary defence** by deflecting Dimorphos, shortening its orbital period by $33.0 \pm 1.0$ minutes ({numref}`fig:l12-dimorphos`) {cite:p}`Thomas2023`, with ejecta recoil giving momentum enhancement $\beta = 3.61^{+0.19}_{-0.25}$ {cite:p}`Cheng2023`.

Equating asteroid gravity to differential solar tides defines the stable **Hill sphere** and yields the Hill radius

$$
r_H = a \left( \frac{m}{3 \, \Msun} \right)^{1/3},
$$

For Didymos ($a \approx 1.64$ AU, mass $m \approx 5.6 \times 10^{11}$ kg), the Hill radius is $r_H \approx 110$ km {cite:p}`Daly2023`.
Dimorphos orbits at $\sim 1.2$ km, well inside the stable limit of $0.5 \, r_H$.
...
Photometric lightcurves of Didymos-Dimorphos six days after DART impact, showing Didymos's 2.26 hr rotation (top) and Dimorphos's new $11.372$ hr orbit (bottom). Kinetic impact reduced the orbital period by $33.0 \pm 1.0$ minutes ($3\sigma$) via ejecta momentum enhancement ({cite:t}`Thomas2023`, Fig. 3).
```

ESA's Hera mission (launched 2024) will survey the crater to refine momentum-transfer efficiency, making planetary defence a quantitative engineering discipline {cite:p}`Michel2022`.
...
The July 2015 New Horizons flyby revealed Pluto as a geologically diverse dwarf planet {cite:p}`Stern2015` ({numref}`fig:l12-pluto-color`).
...
True-colour Ralph instrument image of Pluto acquired by New Horizons during the 2015 flyby, showing the bright heart-shaped Tombaugh Regio and dark equatorial band Cthulhu Macula rich in **tholins**. From Figure 3 of {cite:t}`Stern2015`.
```
...
The most prominent feature is **Sputnik Planitia**, a $\sim 1{,}000$ km wide nitrogen-ice basin where polygonal cells overturn every $\sim 10^{6}$ years via solid-state convection powered by radiogenic decay in the rocky interior {cite:p}`McKinnon2016` ({numref}`fig:l12-sputnik-convection`).
...
Numerical model of solid-state convection in a layer of $\mathrm{N_2}$ ice in Sputnik Planitia, driven at Rayleigh number $\mathrm{Ra}_b \approx 3 \times 10^5$ with overturn timescales of $\sim 10^6$ years. Panels show temperature, surface velocity, dynamic topography, and heat flow across one convective cell (from Figure 4 of {cite:t}`McKinnon2016`).
```
...
The New Horizons flyby of (486958) Arrokoth at $44.6$ AU revealed a **contact binary**: two lobes joined at a narrow neck by a gentle merger under a few m s$^{-1}$ {cite:p}`Stern2019`.
...
Giant planets scattered planetesimals between Jupiter and Neptune onto eccentric orbits, where passing stars and the galactic tide lifted their perihelia into the outer cloud {cite:p}`Dones2004`.
...
Earth's ocean water has a **deuterium-to-hydrogen ratio** (D/H) of $\sim 1.56 \times 10^{-4}$ (the standard mean ocean water value, or VSMOW), which is closely matched by carbonaceous chondrites.
Comets span a factor of 3 in D/H with no clean separation between dynamical classes: 103P/Hartley 2 ({numref}`fig:l12-hartley2`) is near VSMOW {cite:p}`Hartogh2011`, whereas 67P/Churyumov-Gerasimenko is about three times VSMOW {cite:p}`Altwegg2015`.
...
This final part of the lecture surveys recent results from in situ space missions, sample return, and interstellar visitors.

### Rosetta and 67P/Churyumov-Gerasimenko

The European Space Agency's **Rosetta** mission was the first to orbit a comet, escorting 67P/Churyumov-Gerasimenko through perihelion before a controlled descent in 2016 ({numref}`fig:l12-67p`).
...
(101955) Bennu and the OSIRIS-REx sample collection site at three nested zoom levels, showing the $\sim 490$ m rubble-pile asteroid (top) down to the 1.4 m field of view at Nightingale (bottom). From Figure 1 of {cite:t}`Lauretta2024`.
```
...
The Pb-Pb age of CAIs at $4567.30 \pm 0.16$ Myr anchors time zero, with parallel uranium decay chains making the isochron method self-calibrating.
...
Hayabusa2 and OSIRIS-REx returned carbonaceous material from Ryugu and Bennu, linking spectral types to chondrite groups, while DART demonstrated kinetic asteroid deflection.

The new and most unexpected category is **interstellar visitors**.
They are the first physical samples of other planetary systems directly observable in our solar neighbourhood, linking small bodies to the exoplanet science of {ref}`Lecture 13 <lecture13>`.
