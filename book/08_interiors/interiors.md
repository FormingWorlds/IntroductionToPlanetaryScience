...
Schematic diagram of Mars's interior structure inferred from InSight seismology. The cross-section depicts the core-induced shadow zone for seismic waves: a great-circle cut through Olympus Mons from the InSight landing site to the antipodes shows that the S-wave shadow zone is minimal and probably filled by diffracted S-waves (SdiffS), while the P-wave shadow zone is significant and contains specifically the Tharsis region. Annotated values: core radius $1830 \pm 40$ km (Fe-S + light elements, $\rho \approx 5.8\text{-}6.2$ g cm$^{-3}$), no S-waves below the CMB. Reproduced from {cite:p}`Stahler2021`, Fig. 3.
```
...
Radial profiles of Mars compressional ($v_P$) and shear ($v_S$) wave speeds versus depth from three independent InSight inversion approaches: geophysical (blue), geodynamical (orange), and seismic (green). Shaded bands are 50% credible intervals. The $v_P$ profiles are continued into the liquid core below $\sim$800 km depth (right edge of the panel); $v_S$ vanishes there because shear waves do not propagate through the liquid Fe-S outer core. Reproduced from {cite:p}`Stahler2021`, Fig. 2A.
```
...
Jupiter density profiles from a family of Juno-constrained interior models. Coloured curves show $\rho(r)$ for representative dilute-core solutions; the circular schematic at top right indicates the corresponding layered structure: a molecular hydrogen-helium envelope, a metallic hydrogen layer, and a heavy-element gradient (the "dilute" or "fuzzy" core) extending to roughly half the planet's radius rather than being confined to a small compact core. The set of profiles shown all reproduce Juno's measured even gravitational moments $J_2$, $J_4$, $J_6$ within their observational uncertainties, illustrating the family of interior solutions consistent with the data. Reproduced from {cite:p}`Wahl2017`, Fig. 1.
```
