...
$L = R_\mathrm{Moon}$ (show that $\tau \approx 9.6 \times 10^{10}$ yr).
Compare with the age of the solar system.
...
\]
matching the checkpoint. This is about 21 times the age of the solar system (4.57 Gyr):
even the Moon cannot have cooled through its full radius by conduction alone.
\answer{$\tau \approx 9.6 \times 10^{10}$ yr $\gg$ 4.57 Gyr.}
\end{solution}

\parti What is the largest body size that \emph{can} cool conductively within the age of the
solar system ($4.57$ Gyr)? Invert the timescale to find $L$.

\begin{solution}
The age in seconds:
\[
  t = 4.57 \times 10^{9} \times 3.156 \times 10^{7} = 1.4423 \times 10^{17}\ \mathrm{s}.
\]
Setting $\tau = t$ and solving for $L$:
\[
  L = \sqrt{\kappa\,t} = \sqrt{10^{-6} \times 1.4423 \times 10^{17}} = \sqrt{1.4423 \times 10^{11}}
    = 3.798 \times 10^{5}\ \mathrm{m} \approx 380\ \mathrm{km}.
\]
\answer{$L \approx 380$ km: bodies larger than a few hundred kilometres must convect, or stay hot.}
\end{solution}
...
\begin{solution}
From the printed ratio of part (c):
\[
...
$t_c$. Compare with the accretion-timeline figure in the Lecture~4 notes, and state why the
same method cannot date any core-forming event later than $\sim$45 Myr after CAIs.
...
growing. The method fails beyond $\sim$5 half-lives ($\approx 45$ Myr): by then
${}^{182}\mathrm{Hf}$ is effectively extinct, so later core extraction produces no
$\varepsilon^{182}\mathrm{W}$ excess at all, and the clock reads nothing.
\answer{$t_c \approx 3.1$ Myr, in the planetesimal era; after $\sim$45 Myr the parent isotope is extinct and the chronometer is blind.}
\end{solution}
...
\parti If Ganymede's convection stopped, how long would its field survive? Compute the ohmic
decay time $\tau_\mathrm{ohm} = L^2/\eta$ for the 300 km shell, in years. What does the
comparison with the age of the solar system prove about the field Galileo measured in 1996?

\begin{solution}
\[
  \tau_\mathrm{ohm} = \frac{(3.0 \times 10^{5})^2}{1} = 9.0 \times 10^{10}\ \mathrm{s}
  = \frac{9.0 \times 10^{10}}{3.156 \times 10^{7}} = 2852\ \mathrm{yr} \approx 2.9 \times 10^{3}\ \mathrm{yr}.
\]
On any geological timescale this is an instant: the field we observe today is being
regenerated continuously, so Galileo's 1996 detection is proof of a currently operating
dynamo, not of a fossil field.
\answer{$\tau_\mathrm{ohm} \approx 3 \times 10^{3}$ yr: the observed field demands active regeneration now.}
\end{solution}
