...
\vskip4pt
Moon: $R = 1737$ km, $g = 1.62\ \mathrm{m\,s^{-2}}$, simple-to-complex transition $D_{t} \approx 15$ km, core radius $200$--$380$ km
...
Compute its mass and kinetic energy (show that $m \approx 1.005 \times 10^{11}$ kg and $E_k \approx 7.238 \times 10^{18}$ J),
then the diameter of the final crater from the gravity-regime scaling
$$
...
Then invert the relation: what impactor diameter does a $50$ km martian crater require (show that $L \approx 8.5$ km)?
...
$$
Locate the transition on Mars (show that $D_t \approx 6.55$ km), classify the crater from part (a) as simple or complex,
...
On the young branch the linear term dominates, so compute the age from $N \approx b\,T$ (show that $T \approx 2.983$ Gyr).
...
$T_0 = \lambda^{-1}\ln\!\left(1 + N/a\right)$ (show that $T_0 \approx 3.901$ Gyr),
then subtract the linear contribution $b\,T_0$ from $N$ and repeat
(show that $T_1 \approx 3.885$ Gyr).
...
(show that $\rho_c \approx 7024\ \mathrm{kg\,m^{-3}}$ and $\rho_m \approx 3295\ \mathrm{kg\,m^{-3}}$).
...
$$
Evaluate it for your Mercury model (show that $C/MR^2 \approx 0.3511$),
...
\parti Evaluate the mean density of Mars (show that $\bar\rho \approx 3934\ \mathrm{kg\,m^{-3}}$),
then its central pressure (show that $P_c \approx 2.485 \times 10^{10}$ Pa, about $25$ GPa).
...
\parti On Earth, direct S waves are absent everywhere beyond about $104^\circ$ from an earthquake,
while P waves are missing only in a ring between about $104^\circ$ and $140^\circ$.
...
so no direct S wave reaches any station beyond the tangent ray at about $104^\circ$: the S shadow is total.
...
This bending deflects arrivals away from the band between about $104^\circ$ and $140^\circ$,
a shadow of refraction rather than absorption, and beyond $140^\circ$ core-traversing P waves reappear.
...
and a liquid iron-sulphur alloy has a smaller bulk modulus than the overlying solid silicate mantle,
...
of light elements, particularly sulphur, less dense than Earth's outer core.
\end{solution}
