...
Venus: $A = 0.77$, $a = 0.723$ AU, $S = 1.91\,S_0$, $T_\mathrm{surf} = 737$ K
...
\vskip4pt
Relative molecular masses (in $u$): H $= 1$, H$_2 = 2$, CO$_2 = 44$
...
Compute Earth's equilibrium temperature (show that $T_\mathrm{eq} \approx 255$ K).

\begin{solution}
$$
T_\mathrm{eq} = \left[\frac{S_0(1-A)}{4\sigma}\right]^{1/4}
...
\parti Compute Venus's equilibrium temperature from its own albedo and insolation (show that $T_\mathrm{eq} \approx 227$ K),
...
(show that $F_\mathrm{abs} \approx 238$ and $455\ \mathrm{W\,m^{-2}}$).
...
(show that $p_\mathrm{phot} = 200$ Pa and $T_\mathrm{phot} \approx 258$ K).
...
Compute this limit (show that $F_\mathrm{OLR}^\mathrm{max} \approx 253\ \mathrm{W\,m^{-2}}$),
...
the bright cloud deck is sulphuric acid, formed only after the surface water vaporised and the atmosphere transformed.
...
\parti Solve the Rayleigh law for the fraction of the original water that remains (show that $f \approx 4.44 \times 10^{-5}$).
...
\parti Convert to an initial water inventory in global-equivalent-layer depth (show that the initial water is $\approx 450$ m GEL),
and compare it with Earth's ocean.

\begin{solution}
The current inventory is the surviving fraction $f$ of the original, so
$$
d_\mathrm{init} = \frac{d_\mathrm{now}}{f} = \frac{0.02}{4.4444\times10^{-5}} = 450.00\ \mathrm{m\ GEL}.
$$
...
\parti Compute the escape speed at Mars's exobase (show that $v_\mathrm{esc} \approx 4.9\ \mathrm{km\,s^{-1}}$).
...
\parti The Jeans escape parameter for a molecule of mass $m$ is $\lambda = GMm/(k_B T_\mathrm{exo}\,r_\mathrm{exo})$,
...
(show that $\lambda_\mathrm{H} \approx 5.29$, $\lambda_{\mathrm{H_2}} \approx 10.6$, and $\lambda_{\mathrm{CO_2}} \approx 233$).

\begin{solution}
Compute the mass-independent group first:
$$
\frac{GM}{k_B T_\mathrm{exo}\,r_\mathrm{exo}}
...
$$
Multiplying by each molecular mass $m = (\text{relative mass}) \times u$:
$$
...
(show that hydrogen escapes about $150$ times faster as H than as $\mathrm{H_2}$),
...
Compute $T_\mathrm{solar}$ (show that $T_\mathrm{solar} \approx 176$ d) and express it in Mercury years.
...
Estimate the fractional decrease in surface area (show that $\Delta A/A \approx 0.57\%$),
...
Compute its orbital period from Kepler's third law (show that $T \approx 7.7$ h),
