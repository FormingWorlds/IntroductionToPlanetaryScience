...
\vskip4pt
{\footnotesize Atmospheric and escape data as in the Lecture 9 and 10 notes.}
\end{given}
...
Keep the planet at Earth's orbit from part (a), and take $g$ and $\kappa$ from the Constants and data box.
...
\begin{solution}
A global field slows the loss, so the claim names a real effect, but it overstates what a field does.
Jeans escape, the process of parts (b) and (c), is independent of the field, and it cannot remove $\mathrm{CO_2}$ from Mars in any case: the Jeans parameter of $\mathrm{CO_2}$ is so large that thermal escape is negligible.
Mars lost its atmosphere through the non-thermal channels: ion pickup and sputtering by the solar wind, and photochemical escape, in which dissociation gives carbon and oxygen atoms enough speed to leave.
A global field deflects the solar wind and weakens the two solar-wind channels, so a longer-lived field would have slowed the loss; photochemical escape continues on magnetised and unmagnetised planets alike (Lecture 5).
How much a field protects an atmosphere is debated: MAVEN escape rates show that an unmagnetised Mars loses its atmosphere over billions of years, and Venus holds a $92$ bar atmosphere of mostly $\mathrm{CO_2}$ with no detectable field.
The defensible version of the claim is that Mars is too small to hold a thick atmosphere once its field is gone, so a longer-lived field would have slowed the loss without settling what Mars keeps.
\answer{Overstated: a global field weakens the solar-wind channels (ion pickup, sputtering) and would have slowed the loss, but it leaves photochemical escape untouched, Jeans escape of $\mathrm{CO_2}$ is negligible either way, and how much a field protects an atmosphere is debated.}
\end{solution}
...
This problem collects three independent measurements, each marking one step in that divergence.

\parti Mercury's spin period $P_\mathrm{rot}$ and orbital period $P_\mathrm{orb}$ are given in the Constants and data box.
...
\parti As Mercury's interior cooled, its radius shrank by $\Delta R$, given in the Constants and data box.
...
\parti Mercury, the smaller planet, still has a magnetic field; Mars lost its field within its first $400$ to $800$ Myr.
Mercury has no atmosphere; Mars kept a thin one.
In a short paragraph, state what planet size predicts for the cooling rate and the dynamo lifetime,
why Mercury's dynamo does not follow the lifetime prediction,
and what the two planets show about magnetic fields and atmospheric retention.
Use the ideas of cooling rate and escape from this worksheet.

\begin{solution}
A small planet has a large surface-area-to-volume ratio, so it cools fast: the diffusive cooling time scales as $L^2$, and a body half the size cools four times faster.
A dynamo needs a core that keeps losing heat to the mantle, or an inner core that freezes and supplies compositional buoyancy, so size alone predicts that the smaller planet loses its dynamo first.
Mars follows this prediction: its stagnant-lid mantle, depleted in radioactive elements, stopped cooling the core efficiently, no inner core has formed, and the dynamo died after $400$ to $800$ Myr.
Mercury breaks it through its composition: its iron core fills about $83\%$ of the radius, most of it has frozen, and the thin liquid shell that remains still convects as it freezes and drives a weak field, about $1\%$ of Earth's.
The atmospheres do not follow the fields.
Mercury has a field and no atmosphere: it formed close to the Sun with few volatiles, and it is small and hot, so light gases have a low Jeans parameter and leave; only a surface-bounded exosphere remains.
Mars has no global field and a thin atmosphere: its gravity holds $\mathrm{CO_2}$ against thermal escape, as Problem 3 showed, while the solar-wind channels of part 3(d) thinned it over billions of years.
Size sets the cooling rate and, with composition, the dynamo lifetime; volatile supply, gravity and temperature set what an atmosphere keeps.
\answer{Size sets the cooling rate, and with it the dynamo lifetime unless composition intervenes: the core of Mars stopped losing heat and its dynamo died, while the oversized core of Mercury still freezes in a thin liquid shell and keeps a weak field despite the smaller size. Atmospheres follow volatile supply, gravity and temperature rather than the field: Mercury has a field and no atmosphere, Mars has no global field and a thin one.}
\end{solution}
