...
\vskip4pt
Earth: $P_0 = 1.013 \times 10^{5}$ Pa, $\Rearth = 6371$ km, $T_{\mathrm{eff}} = 255$ K, surface $288$ K, air density near the surface $\rho = 1.2\ \mathrm{kg\,m^{-3}}$, $\Omega = 7.29 \times 10^{-5}\ \mathrm{rad\,s^{-1}}$, $\Gamma_d = 9.8\ \mathrm{K\,km^{-1}}$
...
\vskip4pt
{\footnotesize Atmospheric data as in the Lecture 5 notes. Molar masses: $\mathrm{CO_2}$ 44.01, air 28.97 (dimensionless $\mu$, in units of $m_u$). The young Sun had $L/\Lsun = 0.71$ (Lecture 6 notes).}
\end{given}
...
(show that $H_\oplus \approx 8.426$ km and $H_{\mathrm{Titan}} \approx 20.24$ km).
...
\end{align*}
matching the checkpoints and the table in the Lecture 5 notes (8.4 and 20 km).
\end{solution}
...
Evaluate $m_{\mathrm{col}}$ for Earth (show that $m_{\mathrm{col}} \approx 1.033 \times 10^{4}\ \mathrm{kg\,m^{-2}}$),
...
$$
matching the checkpoint: about ten tonnes of air stand on every square metre.
...
\parti Suppose Earth's atmosphere were pure $\mathrm{CO_2}$ at the same temperature and pressure.
Compute the new scale height from your answer to part (a), and state in one sentence what this
change would do to the visible thickness of the atmosphere.

\begin{solution}
Only $\mu$ changes, and $H \propto 1/\mu$:
$$
H_{\mathrm{CO_2}} = H_\oplus\,\frac{28.97}{44.01} = 8.426 \times \frac{28.97}{44.01} = 5.546\ \mathrm{km}.
$$
The heavier gas is held more tightly: the same pressure would be packed into a layer about
two thirds as thick, and the atmosphere would hug the surface more closely.
\end{solution}
...
\parti Compute the solar flux at Venus's orbit (show that $S_{\mathrm{Venus}} \approx 2625\ \mathrm{W\,m^{-2}}$),
...
$$
matching the checkpoint. With $A = 0.77$:
$$
...
\parti In the one-layer greenhouse model of Lecture 5, a fully absorbing layer ($\varepsilon = 1$)
gives $T_s = 2^{1/4}\,T_{\mathrm{eff}}$.
Evaluate this for Earth and compare it with the observed 288 K.
What does the comparison tell you about Earth's actual infrared opacity?

\begin{solution}
$$
T_s = 2^{1/4} \times 255 = 1.1892 \times 255 = 303.2\ \mathrm{K}.
$$
The observed surface temperature, 288 K, lies between $T_{\mathrm{eff}} = 255$ K (no greenhouse) and
303.2 K (perfect single-layer greenhouse).
Earth's atmosphere is therefore a partial absorber: in the notation of the notes, an emissivity
$\varepsilon < 1$, so the real greenhouse sits below the fully opaque limit.
\end{solution}
...
form. The figure below reproduces the saturation curves from the notes; part (d) reads it.

\begin{center}
\includegraphics[width=0.62\linewidth]{figures/psat_curves}
\end{center}

\parti Use the integrated Clausius-Clapeyron equation with the triple point of water as the reference state
to compute the saturation vapour pressure of water at $263$ K
(show that $P_{\mathrm{sat}}(263\ \mathrm{K}) \approx 288.0$ Pa).

\begin{solution}
$$
P_{\mathrm{sat}}(263) = 611\,\exp\!\left[-5400\left(\frac{1}{263} - \frac{1}{273}\right)\right]
= 611\,\exp(-0.75211) = 611 \times 0.47137 = 288.0\ \mathrm{Pa},
$$
matching the checkpoint. Ten kelvin below freezing, the air can hold less than half the vapour it holds at the triple point: this steep decline is the exponential sensitivity derived on the blackboard.
\end{solution}

\parti A parcel at the surface has $T = 293$ K and relative humidity 50\%.
Compute its vapour pressure (show that $P_{\mathrm{sat}}(293\ \mathrm{K}) \approx 2357$ Pa and
...
The dew point is the temperature at which this vapour pressure saturates. Inverting the boxed equation:
$$
...
\parti Sketch the temperature of this rising parcel against altitude from the surface to about 6 km.
Mark the dry adiabat below the cloud base, the lifting condensation level from part (b), and the
shallower moist adiabat above it (a representative value on Earth is $5\ \mathrm{K\,km^{-1}}$).
Label both slopes with numbers.

\begin{solution}
The profile is two straight segments meeting at the LCL:
from $(293\ \mathrm{K},\ 0)$ up to $(282.4\ \mathrm{K},\ 1.08\ \mathrm{km})$ with slope
$-9.8\ \mathrm{K\,km^{-1}}$, then continuing with the shallower moist slope $-5\ \mathrm{K\,km^{-1}}$,
reaching about $258$ K at 6 km.

\begin{center}
\begin{tikzpicture}[x=0.16cm, y=0.75cm]
  % x axis: temperature 250-295 K mapped as (T-250)*0.16cm; y: altitude km
  \draw[->] (0,0) -- (48,0);
  \node[below] at (24,-0.55) {\footnotesize $T$ (K)};
  \draw[->] (0,0) -- (0,6.8);
  \node[left] at (-0.6,6.5) {\footnotesize $z$ (km)};
  \foreach \T in {250, 260, 270, 280, 290}
    \draw ({(\T-250)},0.08) -- ({(\T-250)},-0.08) node[below] {\footnotesize \T};
  \foreach \z in {0, 2, 4, 6}
    \draw (0.3,\z) -- (-0.3,\z) node[left] {\footnotesize \z};
  % dry adiabat: (293,0) to (282.4,1.082)
  \draw[very thick, ipsPrimary] ({(293-250)},0) -- ({(282.4-250)},1.082);
  % moist adiabat: slope 5 K/km up to 6 km: T(6) = 282.4 - 5*(6-1.082) = 257.8
  \draw[very thick, ipsAccent] ({(282.4-250)},1.082) -- ({(257.8-250)},6.0);
  \draw[dashed] (0,1.082) -- ({(282.4-250)},1.082);
  % labels sit in empty regions; thin leaders point at the segments
  \node at (20,0.5) {\footnotesize dry: $-9.8\ \mathrm{K\,km^{-1}}$};
  \draw[->, thin, gray] (28.5,0.5) -- (37.2,0.55);
  \node at (32,4.2) {\footnotesize moist: $-5\ \mathrm{K\,km^{-1}}$};
  \draw[->, thin, gray] (25,4.2) -- (19.6,4.1);
  \node[left] at (0,1.45) {\footnotesize LCL $\approx 1.1$ km};
  \fill ({(282.4-250)},1.082) circle (2.2pt);
\end{tikzpicture}
\end{center}

The kink at the LCL is the cloud base: above it, condensation releases latent heat and slows the cooling.
\end{solution}
...
Here you quantify it for the air above the Netherlands, then judge two claims.

\parti Compute the Coriolis parameter $f = 2\Omega\sin\phi$ for Groningen ($\phi = 53^\circ$N;
show that $f \approx 1.164 \times 10^{-4}\ \mathrm{s^{-1}}$).
...
$$
matching the checkpoint. The associated timescale $1/f \approx 2.4$ hours: any motion lasting longer
...
\parti The pressure in that system drops by 20 hPa over 1000 km.
Compute the geostrophic wind speed from the scalar geostrophic relation of Lecture 6, and compare it
with the quoted $15\ \mathrm{m\,s^{-1}}$.

\begin{solution}
The horizontal pressure gradient is
$$
\left|\pdv{P}{y}\right| = \frac{2000\ \mathrm{Pa}}{10^{6}\ \mathrm{m}} = 2.0 \times 10^{-3}\ \mathrm{Pa\,m^{-1}},
$$
so
$$
u_g = \frac{1}{f\rho}\left|\pdv{P}{y}\right| = \frac{2.0 \times 10^{-3}}{1.164 \times 10^{-4} \times 1.2}
= 14.32\ \mathrm{m\,s^{-1}}.
$$
This agrees with the quoted wind to within a few percent: mid-latitude weather systems really are
geostrophic to good approximation.
\end{solution}

\parti Hurricanes and mid-latitude cyclones spin, yet no cyclone ever forms within a few degrees of the
equator. Explain why, using the latitude dependence of $f$.

\begin{solution}
At the equator $\sin\phi = 0$, so $f = 0$: there is no Coriolis deflection at all.
Without it, air accelerates straight down the pressure gradient and a low fills in instead of spinning up;
no balanced rotating flow can be established.
A developing storm needs a finite $f$ to convert convergence into rotation, which in practice requires
a few degrees of latitude before the Coriolis force is strong enough.
\end{solution}
...
(show that $T_{\mathrm{eff}} \approx 234.1$ K), and state the paradox this number creates
...
$$
matching the checkpoint. With a greenhouse like today's ($+33$ K), the mean surface would sit near
