...
\vskip4pt
Earth: $P_0 = 1.013 \times 10^{5}$ Pa, $\Rearth = 6371$ km, $T_{\mathrm{eq}} = 255$ K (the equilibrium temperature of Lecture 5), surface $288$ K, $\Gamma_d = 9.8\ \mathrm{K\,km^{-1}}$

\vskip4pt
Water: $L_v = 2.50 \times 10^{6}\ \mathrm{J\,kg^{-1}}$, $R_v = 462\ \mathrm{J\,kg^{-1}\,K^{-1}}$ (use $L_v/R_v = 5400$ K throughout), triple point $(273\ \mathrm{K},\ 611\ \mathrm{Pa})$
...
\end{align*}
For Titan, with $T = 94$ K, $\mu = 27.4$, $g = 1.35\ \mathrm{m\,s^{-2}}$:
\begin{align*}
H_{\mathrm{Titan}} &= \frac{1.381 \times 10^{-23} \times 94}{27.4 \times 1.661 \times 10^{-27} \times 1.35}
= \frac{1.2981 \times 10^{-21}}{6.1440 \times 10^{-26}} = 21128\ \mathrm{m} \approx 21.13\ \mathrm{km},
\end{align*}
matching the table in the Lecture 5 notes (8.4 and 21 km).
\answer{$H_\oplus \approx 8.426\ \mathrm{km}$ and $H_{\mathrm{Titan}} \approx 21.13\ \mathrm{km}$, matching the $8.4$ and $21\ \mathrm{km}$ in the Lecture 5 notes.}
\end{solution}
...
\answer{$H \propto T/g$: the gravity ratio $(9.81/1.35 \approx 7.3)$ dominates over the temperature ratio $(288/94 \approx 3.1)$, so $H_{\mathrm{Titan}}/H_\oplus \approx 7.3/3.1 \approx 2.4$. Weak confinement, not warmth, puffs up Titan's atmosphere.}
\end{solution}
...
\[
M_{\mathrm{atm}} = 4\pi \Rearth^2\,m_{\mathrm{col}} = 4\pi\,(6.371 \times 10^{6})^2 \times 1.0326 \times 10^{4} = 5.267 \times 10^{18}\ \mathrm{kg},
\]
about one millionth of Earth's mass.
\answer{$m_{\mathrm{col}} = 1.0326 \times 10^{4}\ \mathrm{kg\,m^{-2}}$ and $M_{\mathrm{atm}} = 5.267 \times 10^{18}\ \mathrm{kg}$, about one millionth of Earth's mass.}
\end{solution}
...
\answer{$S_{\mathrm{Venus}} = 2625.4\ \mathrm{W\,m^{-2}}$ and $T_{\mathrm{eq}} = 227.2\ \mathrm{K}$, matching the notes' $227\ \mathrm{K}$: Venus reflects so much sunlight that its equilibrium temperature is lower than Earth's $255\ \mathrm{K}$.}
\end{solution}
...
\answer{The 92-bar $\mathrm{CO_2}$ atmosphere blocks outgoing thermal infrared from the surface, forcing the ground to radiate far more strongly and heat up to push the absorbed solar flux to space.}
\end{solution}
...
\answer{False: the atmosphere is not a new energy source but recycles energy originally radiated by the surface, conserving energy at every level while slowing its escape to space.}
\end{solution}
...
while its vapour pressure is taken as constant (as instructed; in reality it falls slowly with the total pressure), so condensation begins where the parcel temperature
meets the dew point:
\[
...
\answer{$P_{\mathrm{vap}} = 1179\ \mathrm{Pa}$, $T_d = 282.4\ \mathrm{K}$, and $z_{\mathrm{LCL}} = 1.082\ \mathrm{km}$, sitting in the typical $1$--$2\ \mathrm{km}$ range for Earth's cloud base.}
\end{solution}
...
figure which of the five species is at its saturation point under these conditions, and deduce from the
given data which body in the solar system this is. Justify your reading in one or two sentences.
...
The given-data table quotes 94 K for Titan's mean surface; the methane saturation pressure rises by only a factor of 1.6 from 90 to
94 K, so both temperatures sit within a factor of two of saturation for a $\sim$0.1 bar methane partial pressure, which is
exactly why methane there cycles between vapour, rain, and lakes.
\answer{The species is $\mathrm{CH_4}$ and the body is Titan: methane is at saturation near $10^4\ \mathrm{Pa}$ at $90\ \mathrm{K}$, driving Titan's methane cycle of vapour, rain, and lakes.}
\end{solution}

\parti The Lecture 6 notes state that Venus's sulfuric-acid cloud base is sharply defined, while Titan's
methane clouds can form over a wide range of altitudes. Use the slopes of the saturation curves in the
figure together with the dry lapse rates $\Gamma_d = g/c_p$ of the two atmospheres (Venus: $g = 8.87\ \mathrm{m\,s^{-2}}$,
$c_p \approx 900\ \mathrm{J\,kg^{-1}\,K^{-1}}$, cloud base near 350 K; Titan: $g = 1.35\ \mathrm{m\,s^{-2}}$,
$c_p \approx 1040\ \mathrm{J\,kg^{-1}\,K^{-1}}$, clouds near 90 K) to explain both observations with the same argument.

\begin{solution}
Condensation begins where the local vapour pressure crosses the saturation curve, and the sharpness of a
cloud base in altitude is set by how fast $P_{\mathrm{sat}}$ changes per kilometre of ascent:
\[
\frac{\mathrm{d}\ln P_{\mathrm{sat}}}{\mathrm{d}z} = \frac{L_v}{R_v T^2}\,\Gamma_d .
\]
Per kelvin, the $\mathrm{CH_4}$ curve at 90 K is the steeper one: $980/90^2 = 0.121\ \mathrm{K^{-1}}$ against
$6400/350^2 = 0.052\ \mathrm{K^{-1}}$ for $\mathrm{H_2SO_4}$ at 350 K, so a factor of ten in $P_{\mathrm{sat}}$
needs $\ln 10/0.121 = 19$ K on Titan and $\ln 10/0.052 = 44$ K on Venus.
The lapse rates turn these temperature intervals into altitude: on Venus $\Gamma_d = 8.87/900 \approx 9.9\ \mathrm{K\,km^{-1}}$,
so the factor of ten spans only $44/9.9 \approx 4.5$ km and the transition from clear air to cloud is sharp,
with droplets that settle below the base evaporating within a few kilometres; on Titan
$\Gamma_d = 1.35/1040 \approx 1.3\ \mathrm{K\,km^{-1}}$, so the same factor of ten spans $19/1.3 \approx 15$ km,
and methane stays close to saturation over many kilometres of altitude, where clouds can appear.
One equation, and it is the weak gravity of Titan, not a flat curve, that spreads its clouds.
The factor of ten is a convention: the physical scale is the altitude over which $P_{\mathrm{sat}}$ falls by a
factor $e$, $H_c = R_v T^2/(L_v \Gamma_d)$, and any fixed factor gives the same ratio between the two worlds.
A decade is a useful marker because above it most of the vapour present at the cloud base has condensed
(about 80 percent on both worlds; the fraction is below $1 - 1/10$ because the total pressure, and with it the
saturation mixing ratio, also falls with altitude), so it brackets the layer that holds most of the cloud.
\answer{The saturation pressure changes with altitude at the rate $(L_v/R_v)\,\Gamma_d/T^2$: on Venus a factor of ten in $P_{\mathrm{sat}}$ spans about $4.5\ \mathrm{km}$, so the $\mathrm{H_2SO_4}$ cloud base is sharp; on Titan, with $\Gamma_d \approx 1.3\ \mathrm{K\,km^{-1}}$, it spans about $15\ \mathrm{km}$, so methane clouds form across many kilometres of altitude.}
\end{solution}
...
Compute Earth's equilibrium temperature at that time, taking today's albedo,
and state the paradox this number creates
when combined with the geological record.

\begin{solution}
The equilibrium temperature scales as the fourth root of the absorbed flux, so
\[
T_{\mathrm{eq}} = 255 \times (0.71)^{1/4} = 255 \times 0.91794 = 234.1\ \mathrm{K}.
\]
...
Yet Hadean zircons (4.4 Ga) and Archean pillow basalts and stromatolites record liquid water and, later, life.
A frozen prediction against a wet record: this is the faint young Sun paradox.
\answer{$T_{\mathrm{eq}} = 234.1\ \mathrm{K}$ implies a frozen surface near $267\ \mathrm{K}$ (with a $+33\ \mathrm{K}$ greenhouse), contradicting the Hadean and Archean geological evidence of liquid water and life (the faint young Sun paradox).}
\end{solution}
...
inhabit.
\answer{The outer crossings at $250\ \mathrm{K}$ (snowball) and $287\ \mathrm{K}$ (warm state) are stable, while the middle crossing at $267\ \mathrm{K}$ is unstable; a warm nudge from $267\ \mathrm{K}$ runs away to the stable warm state at $287\ \mathrm{K}$.}
\end{solution}
...
\answer{Elevated $\mathrm{CO_2}$ warms Earth and speeds silicate weathering to bury carbonate until balance is restored; on Mars volcanism ceased about $3\ \mathrm{Gyr}$ ago, breaking the carbon source.}
\end{solution}
