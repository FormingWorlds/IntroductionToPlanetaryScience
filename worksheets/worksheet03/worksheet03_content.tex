...
\vskip4pt
Earth: $P_0 = 1.013 \times 10^{5}$ Pa, $\Rearth = 6371$ km, $T_{\mathrm{eq}} = 255$ K, surface $288$ K, $\Omega = 7.29 \times 10^{-5}\ \mathrm{rad\,s^{-1}}$, $\Gamma_d = 9.8\ \mathrm{K\,km^{-1}}$
...
\answer{$H_\oplus \approx 8.426\ \mathrm{km}$ and $H_{\mathrm{Titan}} \approx 20.24\ \mathrm{km}$, matching the $8.4$ and $20\ \mathrm{km}$ in the Lecture 5 notes.}
\end{solution}
...
\answer{A large $L_v/R_v$ makes the saturation curve steep and the $\mathrm{H_2SO_4}$ cloud base sharp, whereas a low $L_v/R_v$ for $\mathrm{CH_4}$ allows condensation across many kilometres of altitude.}
\end{solution}
...
\parti Compute the Coriolis parameter $f = 2\Omega\sin\phi$ for Groningen ($\phi = 53^\circ\mathrm{N}$).
...
\answer{$f = 1.164 \times 10^{-4}\ \mathrm{s^{-1}}$, giving an inertial timescale $1/f \approx 2.4\ \mathrm{hours}$ beyond which motions feel planetary rotation.}
\end{solution}
...
\answer{$\mathrm{Ro} = 0.064 \ll 1$: rotation dominates over inertia, putting the system in geostrophic balance where the pressure gradient force balances the Coriolis force.}
\end{solution}
...
\answer{False: weak Coriolis deflection allows overturning to run uninterrupted from equator to pole in one giant Hadley cell per hemisphere, creating fewer cells than Earth, not more.}
\end{solution}
...
\answer{$T_{\mathrm{eq}} = 234.1\ \mathrm{K}$ implies a frozen surface near $267\ \mathrm{K}$ (with a $+33\ \mathrm{K}$ greenhouse), contradicting Archean geological evidence of liquid water and life (the faint young Sun paradox).}
\end{solution}
...
greenhouse.
\answer{Both halves are wrong: early Earth had less land, not more, and ocean is darker than land, so smaller continental area lowered the planetary albedo.}
\end{solution}
