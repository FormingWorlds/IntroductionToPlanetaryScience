...
\parti Compute Jupiter's bulk density from its mass and radius (show that $\rho \approx 1326\ \mathrm{kg\,m^{-3}}$).
...
\parti Using the uniform-density result $P_c = 3GM^2/(8\pi R^4)$, estimate Jupiter's central pressure (show that $P_c \approx 1.20 \times 10^{12}$ Pa). Express it in GPa and compare it with the $\sim 100$ GPa metallic-hydrogen transition.
...
\parti Repeat the estimate for Saturn (show that $P_c \approx 2.24 \times 10^{11}$ Pa). Compare the two central pressures against the metallization pressure and state what this implies for the relative size of each planet's metallic-hydrogen region.
...
\parti Compute the rigid-body Roche limit for an icy body at Saturn, $d_R = 1.26\,R_p(\rho_p/\rho_s)^{1/3}$ (show that $d_R \approx 64{,}710$ km). Express it in units of $R_p$.
...
\parti Compute the fluid Roche limit, $d_R = 2.46\,R_p(\rho_p/\rho_s)^{1/3}$ (show that $d_R \approx 126{,}350$ km). Compare it with the observed outer edge of the A ring at $\sim 137{,}000$ km.
...
\parti A ring particle made of rock rather than ice has $\rho_s = 3000\ \mathrm{kg\,m^{-3}}$. Recompute the fluid Roche limit (show that $d_R \approx 87{,}670$ km) and explain why the main rings, which sit near $126{,}350$ km, are icy rather than rocky.
...
\parti For radioactive decay $N = N_0\,e^{-\lambda t}$ with $\lambda = \ln 2 / t_{1/2}$, compute the decay constant of $^{26}$Al (show that $\lambda \approx 0.9667\ \mathrm{Myr^{-1}}$) and the fraction of the initial $^{26}$Al left after $2$ Myr (show that $N/N_0 \approx 0.1446$).
...
After $2$ Myr,
$$
\frac{N}{N_0} = e^{-\lambda t} = e^{-0.9667 \times 2} = e^{-1.9334} = 0.1446.
$$
...
\parti Radiogenic heating scales with the amount of live $^{26}$Al present at accretion. A planetesimal that accretes $2$ Myr after the first solids (CAIs) starts with $^{26}\mathrm{Al}/^{27}\mathrm{Al} = (5.2\times10^{-5}) \times 0.1446$ (show that this is $\approx 7.52\times10^{-6}$). State the factor by which its heating rate is lower than a body that formed at CAI time, and what this implies for melting.

\begin{solution}
$$
\left(\frac{^{26}\mathrm{Al}}{^{27}\mathrm{Al}}\right) = 5.2\times10^{-5} \times 0.1446 = 7.52\times10^{-6}.
$$
The heating rate is proportional to the live $^{26}$Al, so it is lower by the factor $1/0.1446 = 6.9$.
...
$$
Using the CAI age $t = 4.5673$ Gyr and present $^{238}\mathrm{U}/^{235}\mathrm{U} = 137.82$, compute this ratio (show that it is $\approx 0.625$).
...
$$
With $^{235}\mathrm{U}/^{238}\mathrm{U} = 1/137.82 = 7.254\times10^{-3}$,
$$
\frac{^{207}\mathrm{Pb}^*}{^{206}\mathrm{Pb}^*} = 7.254\times10^{-3} \times \frac{88.84}{1.031} = 7.254\times10^{-3} \times 86.17 = 0.6251.
$$
...
\parti Compute the locations of the $5\!:\!2$ and $2\!:\!1$ resonances (show that $a \approx 2.82$ AU and $a \approx 3.28$ AU).
...
\parti Using Kepler's third law, compute the orbital period of an asteroid at the $3\!:\!1$ location $a = 2.501$ AU (show that $T \approx 3.955$ yr), and confirm it is one third of Jupiter's period.
...
\parti For a dark, fast-rotating nucleus ($A \approx 0$) the whole surface reradiates the absorbed sunlight, $S_0(1\,\mathrm{AU}/r)^2 = 4\sigma T^4$. Compute the temperature at $1$ AU (show that $T \approx 278.3$ K) and give the scaling $T(r)$. Evaluate it at $3$ AU.
...
\parti Water ice sublimates efficiently once the nucleus reaches about $150$ K. Find the heliocentric distance where $T(r) = 150$ K (show that $r \approx 3.4$ AU) and interpret it as the onset distance for strong activity.

\begin{solution}
$$
r = \left(\frac{278.3}{150}\right)^2 = (1.855)^2 = 3.442\ \mathrm{AU}.
$$
...
\parti Close to the Sun almost all absorbed sunlight goes into latent heat, so the sublimation rate per unit area is $Z = S_0(1\,\mathrm{AU}/r)^2 / (4Lm)$ molecules per square metre per second. Compute $Z$ at $1$ AU (show that $Z \approx 4.0 \times 10^{21}\ \mathrm{m^{-2}\,s^{-1}}$) and give its scaling with $r$.
