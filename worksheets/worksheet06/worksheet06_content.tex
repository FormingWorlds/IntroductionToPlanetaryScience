...
\vskip4pt
Jupiter: $M = 1.898 \times 10^{27}$ kg, $R = 6.991 \times 10^{7}$ m, semi-major axis $a_\mathrm{J} = 5.203$ AU.\quad
...
\vskip4pt
{\footnotesize The uniform-density central pressure of a self-gravitating sphere in hydrostatic equilibrium is $P_c = 3GM^2/(8\pi R^4)$; the fluid Roche limit is $d_R = 2.46\,R_p(\rho_p/\rho_s)^{1/3}$ and the rigid-body value is $d_R = 1.26\,R_p(\rho_p/\rho_s)^{1/3}$. Data as in the Lecture 8, 11 and 12 notes.}
\end{given}
...
The Roche limit is the distance inside which a moon is tidally torn apart rather than held together by its own gravity.
...
\parti Small moons such as Pan and Daphnis orbit inside the A ring, at about $133{,}600$ km and $136{,}500$ km, outside the fluid limit of part (b). Their own bulk densities are much lower than that of solid ice, about $400$ kg m$^{-3}$, because they are porous rubble piles. Recompute both limits for that density and explain in two or three sentences where the two moons then sit and what the two limits bracket.

\begin{solution}
With $(\rho_p/\rho_s)^{1/3} = (687/400)^{1/3} = 1.198$, the rigid limit becomes $1.26 \times 58{,}232\ \mathrm{km} \times 1.198 = 87{,}900$ km and the fluid limit $2.46 \times 58{,}232\ \mathrm{km} \times 1.198 = 171{,}500$ km.
...
Pan and Daphnis lie between the two: a strengthless body of their density would be torn apart at their orbits, a rigid one would survive, and a real moonlet with modest material strength sits in between, so the two limits bracket the true disruption distance.
\answer{At $400$ kg m$^{-3}$ the limits are $87{,}900$ km and $171{,}500$ km; Pan and Daphnis lie between them, held by gravity plus modest strength, so the rigid and fluid limits bracket the real disruption distance.}
\end{solution}
...
\parti At an interior $j\!:\!k$ resonance the asteroid completes $j$ orbits for every $k$ of Jupiter's, so its period is $T = (k/j)\,T_\mathrm{J}$. Using Kepler's third law $a^3 = T^2$ (in ratio form, $(a/a_\mathrm{J})^3 = (T/T_\mathrm{J})^2$), show that the resonance semi-major axis is $a = a_\mathrm{J}\,(k/j)^{2/3}$, and compute the $3\!:\!1$ location.
