...
\[
  M = \frac{4\pi^2 r^3}{G P^2}.
\]
\answer{$M = 4\pi^2 r^3 / (G P^2)$: the orbit radius and period of a satellite measure the mass of the body it orbits.}
\end{solution}
...
\parti Compute the semi-major axis $a_t$ of the transfer ellipse. Explain why the flight
...
planet librates (oscillates about a mean direction) back and forth in longitude and drags the bulge with it. Both effects flex the
...
\answer{$\Delta a = 2 G M_p R_s / d^3$ and $\Delta a / a = 2 R_s / d$: the tidal difference falls as $d^{-3}$, one power faster than the direct pull.}
\end{solution}
...
($\rho_s = 1000\ \mathrm{kg\,m^{-3}}$). Saturn's
...
$500\ \mathrm{km}$. Compute the escape speed of the smaller body,
