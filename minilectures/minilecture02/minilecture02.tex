...
\begin{frame}{The radiogenic clock}
  ...
  \begin{columns}[T]
    ...
    \begin{block}{Two clocks, two eras}
      ${}^{26}$Al ($t_{1/2} = 0.72$ Myr) out-heats everything for the first few Myr, then dies: planetesimals melt on the short clock, planets stay warm on the long one.
    \end{block}
  \end{columns}
...
\begin{frame}{A minimal thermal evolution model}
  \begin{columns}[T]
    ...
    \[
      \boxed{C\,\frac{\dd T}{\dd t} = H(t) - Q(T)}
    \]
    \begin{itemize}\setlength\itemsep{1pt}
...
\begin{frame}{Dynamos and shields}
  \begin{columns}[T]
    ...
    \begin{block}{The sixth root}
      \small
      A tenfold solar-wind surge moves Earth's magnetopause only from 10 to 6.8 $\Rearth$. Weak dependence is the shield's strength.
    \end{block}
  \end{columns}
  \vskip2pt\relax
