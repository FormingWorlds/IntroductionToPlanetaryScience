...
\begin{frame}{Equilibrium temperature and the habitable zone}
  ...
  \begin{columns}[T]
    ...
    \]
    \begin{itemize}
      \item Earth: $T_\mathrm{eq} \approx 255$ K; the greenhouse effect adds $33$ K.
      \item Sun: $0.97$ to $1.69$ AU; TRAPPIST-1: a few hundredths of an AU.
    \end{itemize}

    \column{0.44\textwidth}
    \begin{block}{Small stars, mixed blessing}
      M dwarfs give deep transits and long lifetimes, but their habitable-zone planets are tidally locked, baked in the pre-main-sequence phase and flared for Gyr. K dwarfs may be the compromise.
    \end{block}
  \end{columns}
  \keyresult{The habitable zone moves as $\sqrt{L_\star}$, the stellar lifetime as $M^{-2.5}$: the trade-offs are quantitative.}
\end{frame}
...
\begin{frame}{Direct imaging and the Drake equation}
  ...
  \begin{columns}[T]
    ...
    \]
    \begin{itemize}
      \item An Earth analogue at $10$ pc: $0.1$ arcsec at $10^{-10}$ contrast. The contrast is the blocker, not the angle.
      \item The M-dwarf habitable zone collapses to milliarcseconds: imaging and transits split the sky by stellar type.
    \end{itemize}

    \column{0.44\textwidth}
    \begin{block}{Drake as a framing tool}
      Four factors log-uniform over ten decades give $\log_{10} N$ a mean of $-20$ and $\sigma \approx 5.8$: the output restates the priors, and ``we are alone'' is consistent with current knowledge.
    \end{block}
  \end{columns}
  \keyresult{Direct imaging is a contrast problem; the Drake equation is a map of ignorance.}
\end{frame}
