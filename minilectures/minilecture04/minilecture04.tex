...
\begin{frame}{What Worksheet 4 trains}
  Four problems, all built on Lectures 7--8. The physics you need, per problem:
  \vskip4pt\relax
  \begin{enumerate}
...
\begin{frame}{Reading ages from craters}
  ...
  \begin{columns}[T]
    ...
    \column{0.44\textwidth}
    \centering
    \includegraphics[width=\linewidth, height=0.38\textheight, keepaspectratio]{neukum_chronology_noreadoff}\\
    {\tiny The chronology curve printed on the sheet. $N(T)$ has no closed-form inverse: on the steep branch neglect the small term, solve, and correct once; on the shallow branch check that the dropped term is small.}
  \end{columns}
  \vskip2pt\relax
...
\begin{frame}{Working the worksheet}
  \begin{itemize}
    ...
    \item Problem 2 reads the chronology figure printed on the sheet; the qualitative parts train the exam skill of arguing from mechanisms.
  \end{itemize}
  \vskip8pt\relax
