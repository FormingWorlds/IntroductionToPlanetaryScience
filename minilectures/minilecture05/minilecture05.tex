% minilecture05.tex: Tutorial 5 mini-lecture: Terrestrial Planets
...
% ── Metadata ────────────────────────────────────────────────
\title[T5 mini-lecture: Terrestrial Planets]{Tutorial 5 Mini-Lecture:\\Terrestrial Planets}
...
\begin{frame}{What Worksheet 5 trains}
  Four problems, all built on Lectures 9--10. The physics you need, per problem:
  \vskip4pt\relax
  \begin{enumerate}
    ...
    \item \textbf{Mercury and Mars: two ways to end}: the spin-orbit resonance, thermal contraction, size against dynamo lifetime.
  \end{enumerate}
  \vskip2pt\relax
...
\begin{frame}{Thermal escape from Mars}
  ...
  \begin{columns}[T]
    ...
    \[
      \boxed{\lambda = \frac{GMm}{\kB T_\mathrm{exo}\,r_\mathrm{exo}}}
...
\begin{frame}{Mercury and Mars: two ways to end}
  ...
  \begin{columns}[T]
    ...
    \column{0.44\textwidth}
    \begin{block}{Why the two ended differently}
      Size sets the cooling rate; cooling sets how long the dynamo runs; gravity sets what the atmosphere keeps. Argue from these three controls in part (c).
    \end{block}
  \end{columns}
  \vspace{-2pt}
