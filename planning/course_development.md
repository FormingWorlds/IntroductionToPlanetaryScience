...
- **Length budget (2026-09-10):** deck 60 frames (from 89; 46 after the cut, then figures added to concept frames, four course-original sketches), notes about 7500 words (from 9501), board sheet 5 live steps with the isothermal column integration stated in the closing box; the closing item is the JWST/MIRI TRAPPIST-1 b secondary eclipse on one hero frame; Worksheet 3 drops its rotating-winds problem and part 5(d) (4 problems, 12 parts) and Mini-lecture 3 its winds frame. Round 2 (2026-09-11): the radius-valley population synthesis and the MAVEN ion plume join as heroes; the two subsections without a figure block cite the figure of the next subsection, so no sketch is needed. Round 3 (2026-09-11): deck 59 frames after the course-wide declutter (one-line hero captions, summary frames folded into a key line under the figure, bullet frames capped at 3, tables and derivation steps exempt); the board setup sketch joins the Goal and strategy frame.
...
- Radiative equilibrium: effective temperature vs. actual surface temperature; the greenhouse effect
- Cloud condensation: Clausius-Clapeyron, dew point, and the lifting condensation level
...
**Format:** Problem set (analytical); 5 problems, 16 parts; two parts read course figures printed on the sheet
...
| 14 | Synthesis & astrobiology | Draft complete (the Wordsworth & Kreidberg circulation schematic, which also serves as the title background, is re-rendered at 400 dpi from the arXiv figure PDF and the Lichtenberg 2025 mass-radius figure is trimmed of a dead band and flattened onto white; the Schwieterman et al. 2018 biosignature-classes figure stays at the largest published form) | Low |
...
- Blackboard derivations and worked examples write out every algebra step, in the notes and on the slides alike: no step is left as an exercise, and no manipulation is skipped with a phrase like "it can be shown". A manipulation too long for one line gets its own displayed line instead of an omission.
...
- **G3** (resolved 2026-09-07, see §16) L10 Fig #24 `wordsworth2016_schematic.avif`: caption says "Cartoon … as conceived by Wordsworth2016", but the figure appears to be a hand-drawn cartoon, not a reproduction from the Wordsworth 2016 Annual Review paper. Either replace with the actual schematic from the paper, or rephrase caption as "cartoon summary of the icy-highlands scenario described by Wordsworth 2016".
...
### 7.7 L9 paywalled-figure batch 2026-04-24 (meta-review §M6, partial closure)

Six L9 §6.5 figures refreshed using Groningen institutional access to Nature and Springer. Covers items A1, A2, A3, A4, A5, A8, and G1 in the meta-review §M6 list:

- **A1 `fig:lyons-oxygen`** — Lyons et al. 2014 (Nature 506, 307). Replaced truncated AVIF (was cropped at 2.0 Ga) with the full-range Fig. 1 covering 4.0 Ga to present day, showing both the Great Oxidation Event and Neoproterozoic Oxygenation Event. Existing caption already matched the new figure; no rewrite needed.
- **A2 `fig:earth-venus-topo`** — Smrekar, Davaille & Sotin 2018 (SSRv 214, 88). Replaced the three-panel Earth/Venus topography/Venus geoid figure with a fresh screenshot that includes the full bottom panel (the geoid panel was previously cut off). Caption unchanged.
- **A3 `fig:goldblatt-runaway`** — Goldblatt et al. 2013 (Nature Geoscience 6, 661). No action: existing AVIF (1297×992) already has all four panels (a–d) and matches the caption precisely. Closed as no-op. Superseded: the four-panel figure is Fig. 3 of Kopparapu et al. 2013, not Goldblatt et al. 2013; notes and deck now carry it as `fig:kopparapu-runaway` / `kopparapu2013_runaway_panels.avif` with Kopparapu attribution, and the 282 W/m² claim is anchored by `fig:goldblatt-spectrum` (A8 below).
- **A4 `fig:hamano-two-types` + A5 `fig:hamano-typeI`** — Hamano, Abe & Genda 2013 (Nature 497, 607). Both figures had substantive mismatches with their L9 captions. Replaced with the correct paper figures (main Fig. 1 for Type-I evolution at 1 AU; main Fig. 4 for the two-types phase diagram with orbital distance vs solidification time panel (a) and final water inventory panel (b)). Captions rewritten accurately: the Type-I caption now specifies 1 AU (was incorrectly "0.7 AU but with low net stellar radiation"); the two-types caption now describes the correct axes (orbital distance + net stellar radiation, with explicit $a_\mathrm{cr} \approx 0.77$ AU critical distance) and cross-references `fig:goldblatt-runaway`.
- **A8 `fig:goldblatt-spectrum`** — Goldblatt et al. 2013 (same paper as A3). Replaced a cropped supplementary-info single-panel with Fig. 3 panel (b) from the main paper: thermal-radiance spectra across the runaway threshold at $T_\mathrm{s} = 280, 310, 340, 370, 400$ K. Caption rewritten to describe the multi-$T_\mathrm{s}$ evolution and to make the 282 W/m² asymptote argument explicit, cross-referencing `fig:goldblatt-runaway`.
- **G1 `fig:venus-terrains`** — Widemann et al. 2023 (SSRv 219, 56). Replaced the low-resolution 998×540 version with a high-resolution 1972×1048 screenshot of Fig. 15 (EnVision Regions of Interest overlaid on Magellan base with terrain classification). Caption rewritten to match the actual legend terms (Plains, Tessera, Deformed terrain, Rift zones, Artemis chasma, Craters), remove the incorrect "coronae (pink ovals)" mention that was not in the figure, cite `IvanovHead2015` for the underlying terrain mapping, and explain the EnVision-RoI framing.
...
### 7.9 L14 figure swaps 2026-04-24: Mulders 2024 true source identified + Lichtenberg 2025 Science mass-radius

Final clean-up round on two L14 figures.
...
**L14 fig:l14:massrad — swapped Wordsworth 2022 -> Lichtenberg 2025 Science (Fig. 2)**: the old `wordsworth2022_mass_radius.avif` was replaced with Fig. 2 from `Lichtenberg, Shorttle, Teske, Kempton 2025 Science 390, eads3360` ("Constraining exoplanet interiors using observations of their atmospheres", DOI 10.1126/science.ads3360). The new figure shows mass-radius with planets colour-coded by equilibrium temperature (temperate/bistable/lava/rock-vapour), symbol-shape-coded by atmospheric-characterisation priority (high/low), and seven EOS compositional tracks (100% Fe, Earth-like, Earth-like + 50 wt% H2O, magma ocean + 0.1 or 5.4 wt% H2O, 100% MgSiO3, and gas-dwarf birth + H/He boil-off). Much richer pedagogical content than the old Wordsworth 2022 figure. New bib entry `Lichtenberg2025` added (ADS-verified bibcode 2025Sci...390S3660L). Old `wordsworth2022_mass_radius.avif` deleted from disk; `Wordsworth2022` bib entry retained because it is still cited for the L14 tidal-couplings figure and in L5.

L14 figure count unchanged (30 figures); no drift in course-wide figure numbering.

### 7.10 L4 content expansion 2026-04-24 (meta-review recommendation #5)

L4 was 47% below the course median word count and 6 figures short of the 25-figure norm. The expansion below closes that gap, implementing all seven priorities from the L4 expansion plan.

**Priority A – Hf-W worked example (~450 words, +1 display eq).** Added a §2.3 sub-block that writes out the two-stage instantaneous-closure age equation $\varepsilon^{182}\mathrm{W}_\mathrm{BSE} = \mathcal{Q}_W \cdot [(\mathrm{Hf}/\mathrm{W})_\mathrm{BSE}/(\mathrm{Hf}/\mathrm{W})_\mathrm{CHUR} - 1] \cdot ({}^{182}\mathrm{Hf}/{}^{180}\mathrm{Hf})_0 \cdot e^{-\lambda t_c}$ explicitly, plugs in Earth's $\varepsilon^{182}\mathrm{W} = +2$ and (Hf/W) enrichment factor of 17 to get $t_c \approx 28$ Myr, and discusses why the full protracted-accretion model age is 30–60 Myr. Worked through Mars ($\varepsilon \approx 0.4$, lower enrichment, 10–20 Myr). Note on why Hf-W cannot date the inner-core nucleation.

**Priority B – Equilibration depth + P-T-$f_{\rm O_2}$ rigor (~600 words, +1 display eq, +1 figure).** New subsection "Equilibration depth and the P-T-$f_{\rm O_2}$ dependence" after §2.2. Introduces the Nernst-type parameterisation $\log_{10} D_i^\mathrm{met/sil} = a_i + b_i/T + c_i P/T + d_i \log f_{\rm O_2}$, discusses how the BSE siderophile depletion pattern constrains the equilibration depth to 40–60 GPa (base of a 1000–1500 km deep magma ocean), and adds a schematic figure `siebert2013_partition_coefficients.avif` (generated for this course, redrawn after `Siebert2013` and `Rubie2015`). New `Siebert2013` bib entry.

**Priority C – Mercury vs Ganymede thin-shell dynamo contrast (~700 words, +1 display eq, +1 table, +1 figure).** New subsection at the end of §8 introducing the $B_\mathrm{dip} \propto \sqrt{\rho_\mathrm{core}\mu_0}(F_q D)^{1/3}$ Christensen-Aubert scaling law and contrasting Mercury and Ganymede side by side (table with core radius, shell thickness, buoyancy driver, surface dipole). Explains why Mercury's weak surface field reflects radial attenuation rather than weak dynamo. Ties Venus's missing field to mantle-dynamics control of core heat flow. New schematic figure `christensen2006_dynamo_scaling.avif` showing numerical-simulation scaling-law collapse with planets labelled. New `ChristensenAubert2006` and `OlsonChristensen2006` bib entries.

**Priority D – Giant-impact mantle-stripping (~500 words, +1 figure).** New subsection §1.3 "Giant impacts and mantle stripping" covering Mercury's anomalous 65–70% core mass fraction via the Benz-Slattery-Cameron collisional stripping hypothesis, Mars's hemispheric dichotomy as a candidate Borealis impact, and Earth-Moon as the opposite net-accretion extreme of the same process. New schematic figure `benz2007_mercury_impact.avif` showing a 4-panel SPH-style cartoon of the Mercury mantle-stripping impact. New `Benz1988`, `Benz2007`, `AndrewsHanna2008`, `Marinova2008` bib entries.

**Priority E – §3 Mantle differentiation figures + prose (~350 words, +2 figures).** Added substantive prose on the adiabat-solidus intersection and upward crystallisation front, plus two new figures: `elkinstanton2012_magma_ocean_crystallisation.avif` (schematic of crystallisation front migrating upward with time, labelling bridgmanite/olivine/urKREEP layers) and `elkinstanton2012_lunar_magma_ocean.avif` (schematic cross-section of the lunar magma-ocean end-state with anorthosite crust, urKREEP, cumulate mantle, ilmenite overturn layer, and Fe core). Expanded text on lunar mantle overturn and Procellarum KREEP Terrane.

**Priority F – Impact erosion rigor (~250 words, +1 display eq).** Expanded §4.3 with the Genda-Abe tangent-plane-blow-off threshold $v_i \gtrsim v_\mathrm{esc}$ and applied it to the Moon-forming impact (threshold marginal → proto-atmosphere loss consistent with secondary outgassing) and to Mars's Borealis impact (above threshold → substantial early atmospheric loss). New `GendaAbe2003` and `GendaAbe2005` bib entries.
...
- All 10 checkers (WS01 to WS07 and the three mock exams) compare each recomputed value with the printed one and exit nonzero on a mismatch; WS01 was the last print-only script.
  The `check-numbers` job in `.github/workflows/book.yml` runs all of them before the book build.
  The LaTeX builds stay local, so a broken frame is still only caught on `make`.
...
## 16. Early Mars climate schematic in L10 (2026-09-07)

The hand-drawn cartoon `wordsworth2016_schematic` (G3 above) is replaced by the course-made two-panel schematic `early_mars_climate_schematic` (script `scripts/figures/L10_mercury_mars/fig_early_mars_climate_schematic.py`), in the book and in the L10 deck.

- Panel (a) is the cold baseline of the icy-highlands hypothesis: a pure CO$_2$-H$_2$O atmosphere at or below about 225 K, snow on the southern highlands, a frozen lowland basin drawn as a depression, CO$_2$ ice clouds with a small net effect.
- Panel (b) is one transient warm excursion, repeated: H$_2$ from volcanism (rate uncertain), serpentinisation or a large impact (drawn as a crater), H$_2$-CO$_2$ collision-induced absorption, high-altitude water-ice clouds as extra warming in some models, snowmelt as a surface stream with tributaries into a lake with a delta and carbonate on the lake floor, and H$_2$ escape as the end of the excursion. The $10^5$ to $10^6$ yr duration is labelled as the value in H$_2$ outgassing models (Wordsworth et al. 2021), not as a measured duration.
- The surrounding text states the 225 K ceiling with its cause (open CO$_2$ spectral windows, weak water-vapour feedback), puts the collision-induced absorption sentence and the H$_2$ sources before the figure, keeps the impact-only proposal as one sentence (it fails to match newer data, Kite and Conway 2024), states the cloud-warming conditions from the same review (particles larger than 10 $\mu$m, warming sensitive to the ice-nuclei abundance), notes that a $10^5$ to $10^6$ yr volcanic H$_2$ episode needs $10^7$ to $10^8$ km$^3$ of magma outgassed in about $10^5$ yr, a rate 100 to 1000 times that of the most intense volcanic events on Earth, moves the alluvial-fan sentence into its own paragraph with the epochs of the valleys and fans named as in the Kite and Carter (2022) captions, and gives the closing timescale paragraph two sources: the Kite (2019) abstract for the durations (longest single river-forming period more than $10^2$ to $10^3$ yr, more than $10^5$ wet years in total, at least 2 river-forming periods) and Kite and Conway (2024) for the valley-network formation time (more than $10^4$ yr, probably longer, with runoff everywhere on the planet, unlike the later wet events), their reading of the record as 7 major transitions (with the alternative of local, asynchronous events not ruled out), some sites dry after 3.6 Ga while rivers still formed after 3 Ga, the peak degassing rate of a single mantle plume head as too small relative to the CO$_2$ and H$_2$O reservoirs for rapid warming, mean-obliquity change as a plausible cause (high obliquity moves polar ice to low latitudes, where it melts more easily), and the open question whether early Mars was warm and wet or cold and icy.
- Bib entries used: `Kite2019` (Space Science Reviews 215, 10), `KiteConway2024` (Nature Geoscience 17, 10), `Kite2025` (Nature 643, 60; author list and issue number checked against Crossref), `Wordsworth2021` (Nature Geoscience 14, 127; arXiv 2103.06736), `KiteEpisodic2021` (PNAS 118, e2101959118; no arXiv preprint).
