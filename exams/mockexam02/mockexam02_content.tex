...
Earth & $1.000\ \mathrm{AU}$ & $R = 6371\ \mathrm{km}$, $\bar{\rho} = 5514\ \mathrm{kg\,m^{-3}}$ \\
Mars & $1.524\ \mathrm{AU}$ & $M = 6.417 \times 10^{23}\ \mathrm{kg}$, $R = 3390\ \mathrm{km}$ \\
Saturn & $9.537\ \mathrm{AU}$, $A = 0.34$ & $M = 5.683 \times 10^{26}\ \mathrm{kg}$, $R = 58\,232\ \mathrm{km}$, effective temperature $95\ \mathrm{K}$, outer edge of the main rings at $136\,800\ \mathrm{km}$ from the centre \\
Titan & & $M = 1.345 \times 10^{23}\ \mathrm{kg}$, $R = 2575\ \mathrm{km}$, upper-atmosphere $T = 150\ \mathrm{K}$ \\
Moon & & $M = 7.342 \times 10^{22}\ \mathrm{kg}$, $R = 1737\ \mathrm{km}$, dayside surface $T = 390\ \mathrm{K}$ \\
Comet & $q = 0.59\ \mathrm{AU}$, $Q = 35.1\ \mathrm{AU}$ & \\
\bottomrule
\end{tabular}
...
\parti \pts{3} Compute the semi-major axis, the eccentricity, and the orbital period of the comet.
...
The heat needed to warm a mass $M$ by $\Delta T$ is $E = M c_p \Delta T$; for rock, $c_p \approx 1200\ \mathrm{J\,kg^{-1}\,K^{-1}}$, and silicate rock starts to melt near $1400\ \mathrm{K}$.

\parti \pts{3} Show that the accretion of Mars released about $4.9 \times 10^{30}\ \mathrm{J}$.
...
\parti \pts{3} Assume for the moment that all of this energy is retained as heat.
...
\begin{solution}
\[
  \Delta T = \frac{E}{M c_p} = \frac{4.864 \times 10^{30}}{6.417 \times 10^{23} \times 1200} = 6317\ \mathrm{K},
\]
...
\answer{$\Delta T \approx 6300\ \mathrm{K}$ if fully retained, far above the $\approx 1400\ \mathrm{K}$ melting point: accretion energy is ample to melt Mars even if only a fraction is kept.}
\end{solution}
...
\parti \pts{4} Show that the two-layer model puts the core radius near $3400\ \mathrm{km}$, then compute the fraction of Earth's mass that resides in the core.
...
\parti \pts{3} Inside a uniform sphere of density $\rho$, the local gravitational acceleration is
\[
...
\[
  d = 2.46\, R_{\mathrm{p}} \left( \frac{\rho_{\mathrm{p}}}{\rho_{\mathrm{m}}} \right)^{1/3},
\]
...
\parti \pts{4} Take the ring particles to be icy material with $\rho_{\mathrm{m}} = 1000\ \mathrm{kg\,m^{-3}}$ as a round value.
Show that the Roche limit lies near $126\,000\ \mathrm{km}$ from Saturn's centre.
...
\begin{solution}
\[
  d = 2.46 \times 5.8232 \times 10^{7} \times \left( \frac{687.1}{1000} \right)^{1/3}
    = 2.46 \times 5.8232 \times 10^{7} \times 0.8823
    = 1.264 \times 10^{8}\ \mathrm{m} = 126\,400\ \mathrm{km},
\]
or about $2.2$ Saturn radii.
The main rings end at $136\,800\ \mathrm{km}$, about $8\%$ beyond this nominal limit (real ring particles are porous, with densities well below that of solid ice, so the true limit lies farther out), while the sizeable moons all orbit well outside it.
...
\answer{$d \approx 126\,000\ \mathrm{km} \approx 2.2\, R_{\mathrm{Saturn}}$: rings inside, moons outside. Tides prevent accretion inside the limit.}
\end{solution}

\parti \pts{3} Compute the sunlight flux Saturn absorbs per square metre of its surface.
Then use the measured effective temperature (data table) to show that Saturn emits about $1.9$ times the power it absorbs, and explain in 1 to 2 sentences where the excess comes from.

\begin{solution}
The solar flux at Saturn is $S = S_\oplus/a^2 = 1361/9.537^2 = 14.96\ \mathrm{W\,m^{-2}}$.
Absorbed per unit surface area, with $A = 0.34$:
\[
  \frac{S}{4}(1 - A) = \frac{14.96}{4} \times 0.66 = 2.468\ \mathrm{W\,m^{-2}}.
\]
...
\]
a factor $4.618/2.468 = 1.87$ above the absorbed flux.
The excess ($\approx 2.1\ \mathrm{W\,m^{-2}}$) is internal heat: slow gravitational contraction, and for Saturn in particular the settling of helium droplets through the metallic hydrogen interior (helium rain), both convert gravitational energy into heat.
\answer{Absorbed $2.47\ \mathrm{W\,m^{-2}}$ versus emitted $4.62\ \mathrm{W\,m^{-2}}$: Saturn radiates $1.9$ times what it absorbs, powered by contraction and helium rain.}
\end{solution}
...
\parti \pts{4} Compute the most probable thermal speeds of $\mathrm{N_2}$ and $\mathrm{H_2}$ at the temperature of Titan's upper atmosphere, and apply the rule of thumb to each gas.
...
\[
  v_{\mathrm{th}} = \sqrt{\frac{2 \times 1.381 \times 10^{-23} \times 150}{4.6508 \times 10^{-26}}}
                  = \sqrt{8.9082 \times 10^{4}} = 298\ \mathrm{m\,s^{-1}},
\]
so $v_{\mathrm{esc}}/v_{\mathrm{th}} = 2640/298.5 = 8.8 > 6$: nitrogen is retained.
For $\mathrm{H_2}$, $m = 3.322 \times 10^{-27}\ \mathrm{kg}$:
\[
  v_{\mathrm{th}} = \sqrt{\frac{4.1430 \times 10^{-21}}{3.322 \times 10^{-27}}}
                  = \sqrt{1.2471 \times 10^{6}} = 1117\ \mathrm{m\,s^{-1}},
\]
so $v_{\mathrm{esc}}/v_{\mathrm{th}} = 2640/1117 = 2.4 < 6$: hydrogen escapes.
...
\answer{$v_{\mathrm{th}}(\mathrm{N_2}) = 298\ \mathrm{m\,s^{-1}}$ (ratio $8.8$: retained), $v_{\mathrm{th}}(\mathrm{H_2}) = 1117\ \mathrm{m\,s^{-1}}$ (ratio $2.4$: lost).}
\end{solution}

\parti \pts{3} The Moon's escape velocity is close to Titan's, yet the Moon has no atmosphere.
...
Titan holds an atmosphere with the same gravity because at $9.5\ \mathrm{AU}$ it is cold; temperature, not just gravity, decides.
\answer{Moon: $v_{\mathrm{esc}} = 2.38\ \mathrm{km\,s^{-1}}$ but $v_{\mathrm{th}}(\mathrm{N_2}) = 481\ \mathrm{m\,s^{-1}}$ at $390\ \mathrm{K}$, ratio $4.9 < 6$: too warm to keep gas that Titan, at the same gravity but $150\ \mathrm{K}$, retains.}
\end{solution}
...
Climate models place it where the stellar flux $S$ lies between about $0.35\, S_\oplus$ (outer edge, maximum greenhouse) and $1.06\, S_\oplus$ (inner edge, runaway greenhouse), with $S_\oplus$ the flux Earth receives.
For a star of luminosity $L$, the flux at distance $a$ (in AU) is
\[
  S = \frac{L}{L_\odot} \frac{S_\oplus}{a^2}.
\]

\parti \pts{3} Compute the inner and outer edge of the Sun's habitable zone, in AU.
Which of the planets of the solar system orbit inside it?

\begin{solution}
Setting $S/S_\oplus = 1/a^2$ equal to the two limits:
\[
  a_{\mathrm{in}} = \sqrt{\frac{1}{1.06}} = 0.971\ \mathrm{AU}, \qquad
...
\answer{$0.971$ to $1.690\ \mathrm{AU}$: Earth and Mars are in the habitable zone, Venus is not.}
\end{solution}

\parti \pts{4} A red dwarf star has $L = 0.02\, L_\odot$ and $M_* = 0.30\ \Msun$.
...
\[
  a_{\mathrm{in}} = \sqrt{\frac{0.02}{1.06}} = 0.1374\ \mathrm{AU}, \qquad
...
\[
  P = \sqrt{\frac{0.1374^3}{0.30}} = \sqrt{8.6473 \times 10^{-3}} = 0.09299\ \mathrm{yr} = 34.0\ \mathrm{d}.
\]
\answer{$0.137$ to $0.239\ \mathrm{AU}$; a planet at the inner edge orbits in $34.0\ \mathrm{d}$. Habitable-zone planets of red dwarfs hug their star.}
\end{solution}
