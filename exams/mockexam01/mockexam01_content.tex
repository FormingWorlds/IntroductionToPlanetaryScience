...
\midrule
Earth & $1.000\ \mathrm{AU}$, $A = 0.30$ & mean surface heat flux $0.092\ \mathrm{W\,m^{-2}}$ \\
Venus & $0.723\ \mathrm{AU}$, $A = 0.77$ & surface $T = 737\ \mathrm{K}$, surface $P = 92\ \mathrm{bar}$, $g = 8.87\ \mathrm{m\,s^{-2}}$, $\mu = 43.4$, $c_p = 1130\ \mathrm{J\,kg^{-1}\,K^{-1}}$ \\
...
Half-life of $^{40}\mathrm{K}$: $1.25\ \mathrm{Gyr}$. Age of the solar system: $4.567\ \mathrm{Gyr}$.}
\end{given}
...
\parti \pts{3} Launch opportunities repeat with the \emph{synodic period} $S$, the time between successive identical alignments of Earth and Venus.
Starting from the angular speeds $2\pi/P$ of the two planets, show that
\[
  \frac{1}{S} = \frac{1}{P_{\mathrm{V}}} - \frac{1}{P_{\mathrm{E}}},
\]
and compute $S$ in days.

\begin{solution}
Venus gains angle on Earth at the rate $2\pi/P_{\mathrm{V}} - 2\pi/P_{\mathrm{E}}$.
The same alignment recurs when the gained angle reaches $2\pi$, after a time $S$:
\[
  \frac{2\pi}{S} = \frac{2\pi}{P_{\mathrm{V}}} - \frac{2\pi}{P_{\mathrm{E}}}
  \quad \Longrightarrow \quad
  \frac{1}{S} = \frac{1}{P_{\mathrm{V}}} - \frac{1}{P_{\mathrm{E}}}.
\]
Numerically, with $P_{\mathrm{V}} = 0.61476\ \mathrm{yr}$ and $P_{\mathrm{E}} = 1\ \mathrm{yr}$:
\[
  \frac{1}{S} = 1.6266 - 1.0000 = 0.6266\ \mathrm{yr^{-1}}
  \quad \Longrightarrow \quad
  S = 1.5958\ \mathrm{yr} = 582.9\ \mathrm{d}.
\]
\answer{$S = 1.596\ \mathrm{yr} = 582.9\ \mathrm{d}$: a launch window to Venus opens about every 19 months.}
\end{solution}

\parti \pts{2} At launch, must Venus lead or trail Earth along its orbit for the spacecraft to meet it at arrival? Compute the required angle.

\begin{solution}
During the flight of $0.39981\ \mathrm{yr}$, Venus sweeps
\[
  360^\circ \times \frac{0.39981}{0.61476} = 234.1^\circ,
\]
while the spacecraft sweeps $180^\circ$ from aphelion to perihelion.
Venus must therefore start $234.1^\circ - 180^\circ = 54.1^\circ$ \emph{behind} the launch point:
it trails Earth by $54.1^\circ$.
\answer{Venus must trail Earth by $54.1^\circ$ at launch.
For an inward transfer the target trails; for an outward transfer (Earth to Mars) it leads.}
\end{solution}
...
\parti \pts{4} In this isothermal approximation the pressure falls with altitude as $P(z) = P_0\, e^{-z/H}$.
...
\parti \pts{4} Below the clouds the temperature profile of Venus is close to dry adiabatic, with lapse rate
\[
  \Gamma = \frac{g}{c_p}.
\]
Compute $\Gamma$ and the temperature at $50\ \mathrm{km}$ altitude.

\begin{solution}
\[
  \Gamma = \frac{8.87}{1130} = 7.850 \times 10^{-3}\ \mathrm{K\,m^{-1}} = 7.85\ \mathrm{K\,km^{-1}},
\]
about $0.8\ \mathrm{K}$ per $100\ \mathrm{m}$ of altitude.
Descending from the surface value:
\[
  T(50\ \mathrm{km}) = 737 - 7.850 \times 50 = 737 - 392.5 = 344.5\ \mathrm{K}.
\]
\answer{$\Gamma = 7.85\ \mathrm{K\,km^{-1}}$; $T(50\ \mathrm{km}) \approx 345\ \mathrm{K}$, near the cloud deck conditions where temperatures approach the habitable range.}
\end{solution}
...
\parti \pts{2} Compute Io's mean density. Is Io made mostly of rock or of ice?
...
\parti \pts{3} Compute Io's mean surface heat flux and compare it with Earth's.

\begin{solution}
\[
  q = \frac{L}{4\pi R^2} = \frac{1 \times 10^{14}}{4\pi (1.821 \times 10^{6})^2}
    = \frac{1 \times 10^{14}}{4.1672 \times 10^{13}} = 2.400\ \mathrm{W\,m^{-2}}.
\]
This is $2.400/0.092 \approx 26$ times Earth's mean surface heat flux, from a body of only $1.5\%$ of Earth's mass.
\answer{$q = 2.40\ \mathrm{W\,m^{-2}}$, about $26$ times Earth's $0.092\ \mathrm{W\,m^{-2}}$.}
\end{solution}
...
\parti \pts{3} The Moon has nearly the same size and density as Io, yet shows no volcanic activity today.
Explain the difference in 2 to 3 sentences.

\begin{solution}
The Moon has no resonance partner forcing its eccentricity, so tidal heating by Earth is negligible today.
Its only remaining heat sources, primordial heat and radioactive decay, are too weak for a body this small: small bodies have a large surface-to-volume ratio and cooled off long ago, which is why lunar mare volcanism ended billions of years ago.
Io stays molten only because the resonance keeps feeding orbital energy into its interior.
\answer{No forced eccentricity means no tidal heating; primordial and radiogenic heat alone cannot keep a Moon-sized body active.}
\end{solution}
...
\parti \pts{5} A lunar mare basalt contains 7 times as many $^{40}\mathrm{Ar}$ atoms as $^{40}\mathrm{K}$ atoms.
...
\parti \pts{3} By what factor was a planet's $^{40}\mathrm{K}$ inventory larger at the formation of the solar system than it is today?
State in 1 sentence what this implies for the heat budgets of young planets.

\begin{solution}
Running the decay law backwards over the age of the solar system:
\[
  \frac{N_0}{N} = 2^{t/t_{1/2}} = 2^{4.567/1.25} = 2^{3.654} = 12.6.
\]
Young planets therefore received an order of magnitude more heating from $^{40}\mathrm{K}$ than planets do today, which contributed to early melting and differentiation.
\answer{A factor of $12.6$.
The total radiogenic heating (all isotopes combined) was about a factor $5$ higher at formation, smaller than the $^{40}\mathrm{K}$ factor because uranium and thorium decay more slowly.}
\end{solution}

\parti \pts{4} A volcanic plain on Mars has the same crater density as the lunar mare from part~(b).
...
\parti \pts{4} On Earth, the carbonate-silicate cycle stabilises the climate over millions of years.
...
\parti \pts{4} A water-rich planet cannot radiate more than roughly $280$ to $310\ \mathrm{W\,m^{-2}}$ from a moist atmosphere (the runaway greenhouse limit).
Judge this claim with a supporting calculation, in 3 to 4 sentences, keeping Mars's albedo unchanged:
\emph{``If Mars orbited at Venus's distance, it would undergo a runaway greenhouse.''}

\begin{solution}
At $a = 0.723\ \mathrm{AU}$, keeping Mars's albedo $A = 0.25$, the absorbed flux would be
\[
  \frac{S_0/a^2}{4}(1 - A) = \frac{2603.6}{4} \times 0.75 = 488.2\ \mathrm{W\,m^{-2}},
\]
well above the runaway limit of $\approx 300\ \mathrm{W\,m^{-2}}$: a water-rich planet at that distance cannot balance its energy budget and its oceans boil into steam.
For \emph{present-day} Mars the claim still fails in one respect: a runaway requires a condensable water reservoir to evaporate, and modern Mars retains only $\sim 25\ \mathrm{m}$ global equivalent layer of water.
Early, water-rich Mars at Venus's distance would run away; today's dry Mars would merely become hot.
\answer{The flux comparison supports a runaway ($488 > 300\ \mathrm{W\,m^{-2}}$), but only for a Mars that still has its water; the claim is true for early Mars and only partially for the present planet.}
\end{solution}
...
\parti \pts{4} Show that the orbital semi-major axis is $a = 0.0451\ \mathrm{AU}$.
...
\parti \pts{4} Compute the stellar constant at the planet's orbit and, assuming a Bond albedo of $A = 0.10$, its equilibrium temperature.
What kind of planet is this?

\begin{solution}
The star is Sun-like, so the flux at $a$ follows from the solar constant:
\[
  S = \frac{S_0}{a^2} = \frac{1361}{0.04512^2} = \frac{1361}{2.0358 \times 10^{-3}} = 6.685 \times 10^{5}\ \mathrm{W\,m^{-2}},
\]
about $490$ times the flux at Earth.
Energy balance:
\[
  T_{\mathrm{eq}} = \left[ \frac{S(1-A)}{4\sigma} \right]^{1/4}
                  = \left[ \frac{6.685 \times 10^{5} \times 0.90}{4 \times 5.670 \times 10^{-8}} \right]^{1/4}
                  = \left( 2.6529 \times 10^{12} \right)^{1/4} = 1276\ \mathrm{K}.
\]
A Jupiter-sized planet at $1280\ \mathrm{K}$ on a $3.5$-day orbit is a \emph{hot Jupiter}.
\answer{$S = 6.69 \times 10^{5}\ \mathrm{W\,m^{-2}}$, $T_{\mathrm{eq}} \approx 1280\ \mathrm{K}$: a hot Jupiter.}
\end{solution}
